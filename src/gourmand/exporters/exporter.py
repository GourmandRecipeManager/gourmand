...
from gourmand.gglobals import DEFAULT_ATTR_ORDER, DEFAULT_TEXT_ATTR_ORDER, REC_ATTR_DIC, TEXT_ATTR_DIC, use_threads
...
from gourmand.threadManager import SuspendableThread
...
class exporter(SuspendableThread, Pluggable):
        ...
        for a in self.text_attr_order:
            ...
            if a == "step":
                ...
                for s in steps:
                    ...
                    if not self.use_ml:
                        txt = xml.sax.saxutils.unescape(s)
                    # if self.convert_attnames:
        ...
        for g, ings in self.rd.order_ings(ingredients):
            ...
            for i in ings:
                ...
                if self._grab_attr_(i, "refid"):
                    self.write_ingref(
                        amount=amount, unit=unit, item=self._grab_attr_(i, "item"), refid=self._grab_attr_(i, "refid"), optional=self._grab_attr_(i, "optional")
                    )
                else:
                    self.write_ing(
                        amount=amount, unit=unit, item=self._grab_attr_(i, "item"), key=self._grab_attr_(i, "ingkey"), optional=self._grab_attr_(i, "optional")
                    )
            if g:
    ...
    @pluggable_method
    def write_ingref(self, amount=1, unit=None, item=None, optional=False, refid=None):
        """By default, we don't handle ingredients as recipes, but
        someone subclassing us may wish to do so..."""
        self.write_ing(amount=amount, unit=unit, item=item, key=None, optional=optional)
...
class exporter_mult(exporter):
    ...
    def _get_amount_and_unit_(self, ing):
        if self.mult != 1 and self.change_units:
            return self.rd.get_amount_and_unit(ing, mult=self.mult, conv=self.conv, fractions=self.fractions)
        else:
            return self.rd.get_amount_and_unit(ing, mult=self.mult, conv=self.conv, fractions=self.fractions)
...
class ExporterMultirec(SuspendableThread, Pluggable):
    ...
    def __init__(
        self, rd, recipes, out, one_file=True, create_file=True, ext="txt", conv=None, imgcount=1, exporter=exporter, exporter_kwargs={}, padding=None
    ):
