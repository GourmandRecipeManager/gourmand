...
class Importer (SuspendableThread):
    ...
    def start_rec(self, dict=None):
        ...
        if hasattr(self, 'added_ings') and self.added_ings:
            print(
                'WARNING: starting new rec, but we have ingredients that we never added!')
    ...
    def _move_to_instructions(self, recdic, attr):
        ...
        text_to_add = gourmand.gglobals.REC_ATTR_DIC['yields' if attr == 'servings'
                                                     else attr] \
            + ': '+recdic[attr]
    ...
    def commit_rec(self):
        ...
        if self.rec.get('image', None) and not self.rec.get('thumb', None):
            if not self.rec['image']:
                ...
                if img:
                    ...
                    print("ODD: we got no image from ",
                          self.rec['image'][:100])
        ...
        if id_to_convert:
            if self.rec['id'] in self.id_converter:
                ...
                # See doc on add_rec
                r = self.rd.add_rec(self.rec, accept_ids=True)
            else:
                del self.rec['id']
        ...
        if remembered_rating:
            self.rating_converter.add(r.id, remembered_rating)
        self.check_for_sleep()
        timeaction.end()
        self.rec_timer.end()
        self.count += 1
    ...
    def parse_yields(self, str):
        ...
        m = re.match(
            r"(?P<prefix>\w+\s+)?(?P<num>[0-9/. ]+)(?P<unit>\s*\w+)?", str)
        if not m:
            return None, None
        ...
        if not unit:
            ...
            if prefix not in ['Makes', 'Yields']:
                print(
                    'Warning: parse_yields ignoring prefix, "%(prefix)s" in "%(str)s"' % locals())
        if not unit:
            unit = 'servings'  # default/fallback
        return num, unit
    ...
    def finish_ing(self):
        ...
        if (
            ...
            self.ing['amount'], self.ing['rangeamount'] = parse_range(
                self.ing['amount'])
...
def parse_range(number_string):
    ...
    if len(nums) > 2:
        debug('WARNING: String %s does not appear to be a normal range.' %
              number_string, 0)
...
class RatingConverter:
    ...
    def add(self, id, rating):
        if isinstance(rating, int):
            raise Exception(
                "Why do you need me? id: %(id)s rating: %(rating)s" % locals())
    ...
    def get_conversions(self, star_generator=None):
        ...
        for v in list(self.to_convert.values()):
            ...
            if v not in ratings:
                ...
                if conv:
                    self.conversions[v] = conv
        if need_conversions:
            self.conversions = de.get_ratings_conversion(
                ratings, star_generator, defaults=self.conversions)
        ...
        for id, rating in list(self.to_convert.items()):
            try:
                if rating not in self.conversions and hasattr(rating, 'lower') and rating.lower() in self.conversions:
                    rating = rating.lower()
                db.modify_rec(db.get_rec(id), {
                              'rating': self.conversions[rating]})
            except:
...
class ImporterTest (unittest.TestCase):
    ...
    def testParseComplexYields(self):
        assert(self.importer.parse_yields(
