...
class ShopEditor:
    ...
    def __init__(self, rd=db.recipeManager(), rg=None):
        ...
        self.ui.add_from_string(
            get_data('gourmet', 'ui/shopCatEditor.ui').decode())
        self.rd = rd
        self.rg = rg
    ...
    def tree_edited(self, renderer, path_string, text, n, head):
        ...
        iter = self.filteredModel.convert_iter_to_child_iter(
            self.filteredModel.get_iter(path))
        ...
        if not self.dont_ask:
            ...
            if n == self.ITEM_COL:
                msg += 'item'
            msg += "for \"%s from \"%s\"" % (item,
                                             key) if item else " from \"%s\" " % key
            msg += " to \"%s\"" % text
    ...
    def makeTreeModel(self):
        ...
        unique_cat_vw = self.rd.shopcats_table.groupby(
            self.rd.shopcats_table.category, 'groupvw')
        self.cat_to_key = {}
        for c in unique_cat_vw:
            iter = self.treeModel.append(
                None, [c, pickle.loads(c.category), ""])
            ...
            for i in c.groupvw:
                ...
                self.treeModel.append(
                    iter, [i, pickle.loads(c.category), i.ingkey])
