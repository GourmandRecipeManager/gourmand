...
def fix_colnames(dict, *tables):
    ...
    for k, v in list(dict.items()):
        ...
        if not got_prop:
            raise ValueError(
                "Could not find column %s in tables %s" % (k, tables))
    return newdict
        ...
        if col == 'count' and not hasattr(table.c, 'count'):
            ...
            for t in join_tables:
                if hasattr(t.c, col):
                    ...
                    break
            if broken:
                raise ValueError("No such column for tables %s %s: %s" % (
...
class RecData (Pluggable):
    ...
    def _setup_object_for_table(self, table, klass):
        ...
        if True in [col.primary_key for col in table.columns]:
            ...
            raise Exception(
                "All tables need a primary key -- specify 'rowid'/Integer/Primary Key in table spec for %s" % table)
    ...
    def setup_info_table(self):
        self.info_table = Table('info', self.metadata,
                                # three part version numbers 2.1.10, etc. 1.0.0
    ...
    def setup_ingredient_table(self):
        self.ingredients_table = Table('ingredients', self.metadata,
                                       ...
                                       Column('refid', Integer, ForeignKey(
                                           'recipe.id'), **{}),
                                       Column('unit', Text(), **{}),
                                       Column('amount', Float(), **{}),
    ...
    def backup_db(self):
        ...
        backup_file_name = self.filename + \
            '.backup-' + time.strftime('%d-%m-%y')
    ...
    def update_version_info(self, version_string: str):
        ...
        elif not (stored_info.version_super or stored_info.version_major):
            # Default info -- the last version before we added the
            # version tracker...
            default_info = {'version_super': 0,
                            'version_major': 11,
        ...
        if not self.new_db:
            ...
            if (stored_info.version_super == 0 and ((stored_info.version_major <= 14 and stored_info.version_minor <= 7)
                ...
                self.add_column_to_table(
                    self.recipe_table, ('yield_unit', String(length=32), {}))
            ...
            if stored_info.version_super == 0 and stored_info.version_major <= 12:
                ...
                self.add_column_to_table(
                    self.recipe_table, ('recipe_hash', String(length=32), {}))
                self.add_column_to_table(
                    self.recipe_table, ('ingredient_hash', String(length=32), {}))
                # Add a link field...
                self.add_column_to_table(
    ...
    def fetch_join(self, table1, table2, col1, col2,
        ...
        if column_names:
            raise Exception(
                "column_names KWARG NO LONGER SUPPORTED BY fetch_join!")
    ...
    def search_recipes(self, searches, sort_by=[]):
        ...
        if 'category' in [s[0] for s in sort_by]:
            return sqlalchemy.select([c for c in self.recipe_table.c],  # + [self.categories_table.c.category],
                                     ...
                                     from_obj=[sqlalchemy.outerjoin(
                                         self.recipe_table, self.categories_table)],
    ...
    def get_unique_values(self, colname, table=None, **criteria):
        ...
        criteria = make_simple_select_arg(criteria, table)[
            0] if criteria else None
        ...
        retval = [r[0] for
                  r in sqlalchemy.select([getattr(
                      table.c, colname)], distinct=True, whereclause=criteria).execute().fetchall()
                  ]
    ...
    def update_by_criteria(self, table, update_criteria, new_values_dic):
        try:
            ...
            table.update(*make_simple_select_arg(update_criteria,
                         table)).execute(**new_values_dic)
        except:
    ...
    def alter_table(self, table_name, setup_function, cols_to_change={}, cols_to_keep=[]):
        ...
        tables
        """
        print('Attempting to alter ', table_name,
              setup_function, cols_to_change, cols_to_keep)
        try:
            self.db.execute('ALTER TABLE %(t)s RENAME TO %(t)s_temp' % {
                            't': table_name})
        except:
            do_raise = True
            import traceback
    ...
    def find_duplicates(self, by='recipe', recipes=None, include_deleted=True):
        ...
        if not include_deleted:
            args.append(self.recipe_table.c.deleted == False)
        ...
        if recipes:
            ...
            results = [reclist for reclist in results if True in [
                (rid in rec_ids) for rid in reclist]]
        return results
    ...
    def find_complete_duplicates(self, recipes=None, include_deleted=True):
        ...
        if not include_deleted:
            select_statements = [self.recipe_table.c.deleted == False]
        else:
            select_statements = []
        select_statements.append(
            self.recipe_table.c.ingredient_hash.in_(ing_hashes))
        select_statements.append(
            self.recipe_table.c.recipe_hash.in_(rec_hashes))
    ...
    def modify_rec(self, rec, dic):
        ...
        if 'category' in dic:
            ...
            # Make sure our categories are not blank
            newcats = [x for x in newcats if x]
            curcats = self.get_cats(rec)
            for c in curcats:
                if c not in newcats:
                    self.delete_by_criteria(self.categories_table, {
    ...
    def update_hashes(self, rec):
        ...
        self.do_modify_rec(
            rec, {'recipe_hash': rhash, 'ingredient_hash': ihash})
    ...
    def find_duplicates_of_rec(self, rec, match_ingredient=True, match_recipe=True):
        ...
        if match_ingredient and match_recipe:
            perfect_matches = self.fetch_all(
                ingredient_hash=rec.ingredient_hash, recipe_hash=rec.recipe_hash)
        elif match_ingredient:
            perfect_matches = self.fetch_all(
                ingredient_hash=rec.ingredient_hash)
        else:
    ...
    def add_rec(self, dic, accept_ids=False):
        ...
        try:
            ...
            for k, v in list(dic.items()):
                print('KEY:', k, 'of type', type(k),
                      'VALUE:', v, 'of type', type(v))
            raise
        else:
            ...
            for c in cats:
                if c:
                    self.do_add_cat({'recipe_id': ID, 'category': c})
    ...
    def do_add_and_return_item(self, table, dic, id_prop='id'):
        ...
        select = table.select(getattr(table.c, id_prop) ==
                              result_proxy.inserted_primary_key[0])
    ...
    def do_add_rec(self, rdict):
        ...
        if 'id' in rdict:
            ...
            if rdict['id'] not in self.new_ids:
                raise ValueError(
                    'New recipe created with preset id %s, but ID is not in our list of new_ids' % rdict['id'])
            rid = rdict['id']
            del rdict['id']
        ...
        select = self.recipe_table.select(
            self.recipe_table.c.id == insert_statement.execute(**rdict).inserted_primary_key[0])
        return select.execute().fetchone()
    ...
    def do_modify(self,
        ...
        if id_col is not None:  # Saving a particular entry in the recipe
            try:
                ...
                print(e)
                raise
            select = table.select(getattr(table.c, id_col)
                                  == getattr(row, id_col))
    ...
    def remove_ing_from_keydic(self, item, key):
        ...
        for word in item.split():
            ...
            if row:
                new_count = row.count - 1
                if new_count:
                    self.do_modify(self.keylookup_table,
                                   row, {'count': new_count})
                else:
    ...
    def get_dict_for_obj(self, obj, keys):
        orig_dic = {}
        for k in keys:
            v = ", ".join(self.get_cats(
                obj)) if k == 'category' else getattr(obj, k)
            orig_dic[k] = v
        return orig_dic
    ...
    def undoable_modify_rec(self, rec, dic, history=[], get_current_rec_method=None,
        ...
        reundo_name += ''.join(["%s <i>%s</i>" % (k, v)
                               for k, v in list(orig_dic.items())])
        reapply_name += ''.join(["%s <i>%s</i>" % (k, v)
                                for k, v in list(dic.items())])
        ...
        obj = Undo.UndoableObject(action, action, history,
                                  action_args=[rec, dic], undo_action_args=[
                                      rec, orig_dic],
    ...
    def undoable_delete_ings(self, ings, history, make_visible=None):
        ...
        def do_delete():
            if make_visible:
                modded_ings = [self.modify_ing(
                    i, {'deleted': True}) for i in ings]
                make_visible(modded_ings)

        def undo_delete():
            if make_visible:
                modded_ings = [self.modify_ing(
                    i, {'deleted': False}) for i in ings]
                make_visible(modded_ings)
        obj = Undo.UndoableObject(do_delete, undo_delete, history)
        obj.perform()
        ...
        if m:
            ...
            if u:
                ...
                if conv and u.strip() in conv.unit_dict:
                    ...
                    prev_uses = self.rd.fetch_all(
                        self.rd.ingredients_table, unit=u.strip())
                    if prev_uses:
                        d['unit'] = u
                    else:
...
class dbDic:
    ...
    def values(self):
        return [getattr(i, self.vp) for i in self.db.fetch_all(self.vw)]
