...
for tag, label in [("servings", _("Servings")), ("ingredients", _("Ingredients")), ("inggroup", _("Ingredient Subgroup")), ("ignore", _("Hide"))]:
    if tag not in DEFAULT_TAG_LABELS:
        DEFAULT_TAG_LABELS[tag] = label

UI_TAG_ORDER = [
    (_("Description"), [("title", "servings"), ("yields", "yield_unit"), ("source", "rating"), ("category", "cuisine"), ("preptime", "cooktime")]),
    (_("Text"), [("instructions", "modifications")]),
    (_("Ingredients"), [("ingredients", "inggroup")]),
    (_("Actions"), [("ignore", "clear"), ("newrec",)]),
]
...
class InteractiveImporter(ConvenientImporter, NotThreadSafe):
    ...
    def __init__(self, custom_parser=None, tags=DEFAULT_TAGS, tag_labels=DEFAULT_TAG_LABELS, modal=True, title=_("Import recipe")):
        self.title = title
        if custom_parser:
    ...
    def setup_window(self):
        ...
        from gourmand.threadManager import get_thread_manager_gui

        _ = get_thread_manager_gui()
        self.w = Gtk.Window()
    ...
    def setup_action_area(self):
        ...
        self.action_area.pack_start(action_table, expand=False, fill=False, padding=0)
            ...
            for row in rows:
                for c, t in enumerate(row):  # column number, tag
                    if t == "clear":
                        ...
                        tag_btn.connect("clicked", self.label_callback, self.labels_by_tag[t])
        ...
        self.import_button.connect("clicked", lambda *args: self.commit_changes())

        self.action_area.pack_end(self.import_button, fill=False, expand=False, padding=0)
    ...
    def insert_with_label(self, st, text, label):
        ...
        self.label_range(self.tb.get_iter_at_offset(start_offset), self.tb.get_iter_at_offset(end_offset), label)

    def unhide_area(self, midno):
        st, end = self.markup_marks[midno]
        self.tb.remove_tag(self.ignore_tag, self.tb.get_iter_at_mark(st), self.tb.get_iter_at_mark(end))
    ...
    def remove_markup_text(self, idno):
        ...
        sitr, eitr = (self.tb.get_iter_at_mark(smark), self.tb.get_iter_at_mark(emark))
    ...
    def clear_tags(self, *args):
        ...
        for idno, iters in list(self.markup_marks.items()):
            lst, lend = iters
            if (e_offset > self.tb.get_iter_at_mark(lst).get_offset() > st_offset) or (e_offset > self.tb.get_iter_at_mark(lend).get_offset() > st_offset):
        ...
        if hasattr(self, "images") and self.images:
            for rec in self.added_recs:
                ...
                if response == Gtk.ResponseType.OK:
                    ...
                    self.rd.modify_rec(rec, {"image": image_to_bytes(browser.image), "thumb": image_to_bytes(thumb)})
    ...
    def set_parsed(self, parsed):
        ...
        for chunk, tag in parsed:
            ...
            if tag is None:
                ...
                self.insert_with_label(self.tb.get_end_iter(), chunk, self.labels_by_tag.get(tag, tag))
        # dbg_file.close()
...
def _get_images(soup: BeautifulSoup):
    ...
    for image in soup.find_all("img"):
        if image.attrs.get("src", "").startswith("http"):
            yield image["src"]
...
def import_interactivally(uris: List[str]):
    ...
    for uri in uris:
        ...
        importer.images = list(_get_images(soup))
