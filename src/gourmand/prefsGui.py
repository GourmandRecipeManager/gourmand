...
class PreferencesGui (plugin_loader.Pluggable):
    ...
    def __init__(
        ...
        self.pref_tables = {}
        self.ui.get_object('close_button').connect(
            'clicked', lambda *args: self.hide_dialog())
    ...
    def connect_toggle_buttons(self):
        ...
        for pref, widget in list(self.toggle_options.items()):
            self.ui.get_object(widget).connect(
                'toggled', self.toggle_callback, pref)
    ...
    def add_pref_table(self, options, target_widget, callback=None):
        ...
        """
        table = optionTable.OptionTable(
            options=options, changedcb=self.preftable_callback)
