...
from gourmand.plugin import IngredientControllerPlugin, RecEditorModule, RecEditorPlugin
from gourmand.plugin_loader import POST
...
class IngredientKeyEditor(RecEditorModule):
    ...
    def setup_main_interface(self):
        ...
        label.set_markup(
            """<b>%s</b>\n<i>%s</i>"""
            % (_("Ingredient Keys"), _("Ingredient Keys are normalized ingredient names used for shopping lists and for calculations."))
        )
        ...
        ew_index = 1
        self.main.pack_start(self.extra_widget_table, expand=False, fill=False, padding=0)
        self.tv = Gtk.TreeView()
        self.tv.get_selection().set_mode(Gtk.SelectionMode.MULTIPLE)
        self.tv.get_selection().connect("changed", self.treeselection_changed_cb)
        self.setup_model()
        self.setup_tree()
        ...
        self.keyEditorActionGroup.add_actions(
            [
                (
                    ...
                    _("Guess keys"),
                    None,
                    _("Guess best values for all ingredient keys based on values already in your database"),
                    self.guess_keys_cb,
                ),
            ]
        )
        self.keyEditorActionGroup.add_toggle_actions(
            [
                (
                    "EditAssociations",
                    None,
                    _("Edit Key Associations"),
                    None,
                    _("Edit associations with key and other attributes in database"),
                    self.edit_associations_cb,
                    False,
                ),
            ]
        )
    ...
    def setup_tree(self):
        ...
        plugin_manager = keyEditorPluggable.get_key_editor_plugin_manager()
        for tvc in plugin_manager.get_treeview_columns(self, key_col=2, instant_apply=False):
...
class KeyEditorIngredientControllerPlugin(IngredientControllerPlugin):
    ...
    def activate(self, pluggable):
        pluggable.add_hook(POST, "get_extra_ingredient_attributes", self.get_extra_ingattributes_post_hook)

    def get_extra_ingattributes_post_hook(self, retval: Optional[Any], controller, ing_obj: int, ingdict: Dict[str, Any]) -> Dict[str, Any]:
