...
#from gourmand.gglobals import gladeCustomHandlers
from gourmand.plugin import RecDisplayModule, RecDisplayPlugin
...
class NutritionDisplayModule (RecDisplayModule):
    ...
    def setup_ui(self):
        ...
        self.ui = Gtk.Builder()
        self.ui.add_from_file(os.path.join(
            current_path, 'nut_recipe_card_display.ui'))
        self.ui.connect_signals(
            {'edit_nutrition': lambda *args: self.nutritionLabel.show_druid(
                nd=self.recipe_display.rg.rd.nd)}
        )
        self.nutritionLabel = self.ui.get_object('nutritionLabel')
        self.nutritionLabel.set_prefs(self.prefs)
        self.nutritionLabel.connect(
            'ingredients-changed', self.ingredients_changed_cb)
        self.nutritionLabel.connect(
            'label-changed', self.nutrition_highlighting_label_changed)
        self.main = self.ui.get_object('nutritionDisplay')
        self.main.unparent()

    def ingredients_changed_cb(self, *args):
        self.recipe_display.reccard.update_recipe(
            self.recipe_display.current_rec)
    ...
    def setup_ingredient_display_hooks(self):
        ...
        self.ingredientDisplay.markup_ingredient_hooks.append(
            self.nutritional_markup_hook)
    ...
    def nutritional_markup_hook(self, istr, ing, ing_index, group_index):
        if self.nutritional_highlighting and self.nutritionLabel.active_name:
            ...
            if nut_amt:
                ...
                if self.recipe_display.yields_orig:
                    nut_amt = nut_amt/self.recipe_display.yields_orig
                ...
                color = "#%02x%02x%02x" % (255, 255,
                                           faintest_yellow -
                                           int(faintest_yellow*(perc**2))
                                           )
                nut_highlighted = True
                weight = int(Pango.Weight.NORMAL +
                             ((Pango.Weight.HEAVY - Pango.Weight.NORMAL) * perc))
                if color:
