...
class GourmandApplication:
    ...
    def show_progress_dialog(self, thread, progress_dialog_kwargs=None,
        ...
        self.progress_dialog.connect(
            'close', lambda *args: setattr(self.progress_dialog, None))
    ...
    def add_uimanager_to_manage(self, id, uimanager, menu_root_name):
        ...
        uimanager.add_ui_from_string(self.shared_go_menu % {
                                     'name': menu_root_name})
    ...
    def update_go_menu_for_ui(self, id):
        ...
        for rc in list(self.rc.values()):
            path = self.go_path % {'name': menu_root_name} + \
                'GoRecipe'+str(rc.current_rec.id)
    ...
    def quit(self):
        ...
        for r in list(self.rc.values()):
            ...
            if r.edited and de.getBoolean(parent=self.app,
                ...
                # in case someone else checks this (e.g. reccard on close)
                r.edited = False
        ...
        self.rd.delete_by_criteria(
            self.rd.ingredients_table, {'deleted': True})
...
class SuspendableDeletions (SuspendableThread):
    ...
    def do_run(self):
        ...
        for n, r in enumerate(self.recs):
            ...
            self.emit('progress', float(n)/tot,
                      _('Permanently deleted %s of %s recipes') % (n, tot))
...
class RecTrash (RecIndex):
    ...
    def __init__(self, rg):
        ...
        self.ui.add_from_string(
            get_data('gourmand', 'ui/recipe_index.ui').decode())
...
class ImporterExporter:
    ...
    def do_export(self, export_all=False):
        if export_all:
            recs = self.rd.fetch_all(
                self.rd.recipe_table, deleted=False, sort_by=[('title', 1)])
        else:
            recs = self.get_selected_recs_from_rec_tree()
        ...
        for r in rr:
            if r.yields and r.yields != "None":
                ...
                if not mult:
                    debug('getNumber cancelled', 2)
                    return
            d = self.sl.getOptionalIngDic(
                self.rd.get_ings(r), mult, self.prefs)
...
class RecGui (RecIndex, GourmandApplication, ImporterExporter, StuffThatShouldBePlugins, plugin_loader.Pluggable, BaseException):
    ...
    def setup_column_display_preferences(self, *args):
        already_hidden = self.prefs.get(
            'rectree_hidden_columns', DEFAULT_HIDDEN_COLUMNS)
    ...
    def setup_database_hooks(self):
        self.rd.delete_hooks.append(
            lambda self, *args: (self.doing_multiple_deletions ==
                                 False and self.redo_search())
        )
    ...
    def rectree_popup(self, tv, event, *args):
        menu = self.ui_manager.get_widget(
            "/RecipeIndexMenuBar/Actions/").get_submenu()
    ...
    def setup_actions(self):
        ...
        self.mainActionGroup.add_actions([
            ...
            ('Plugins', None, _('_Plugins'),
             None, _('Manage plugins which add extra functionality to Gourmand.'),
        ...
        if rec.id in self.rc:
            ...
            if rc.edited:
                ...
                if not de.getBoolean(
                        label=_('Delete %s?'),
                        sublabel=_(
                            'You have unsaved changes to %s. Are you sure you want to delete?'),
    ...
    def rec_tree_delete_recs(self, recs):
        cancelled = [
            rec for rec in recs if self.delete_open_card_carefully(rec)]
        if cancelled:
            for c in cancelled:
                recs.remove(c)
        ...
        self.message(_("Deleted") + ' ' +
                     ', '.join((r.title or _('Untitled')) for r in recs))
    ...
    def purge_rec_tree(self, recs, paths=None, model=None):
        ...
        if len(recs) == 1:
            ...
            msg = _(
                "Are you sure you want to delete the recipe <i>%s</i>") % recs[0].title
        elif len(recs) < 5:
            msg = _("Are you sure you want to delete the following recipes?")
            for r in recs:
                msg += "\n<i>%s</i>" % r.title
        else:
            msg = _("Are you sure you want to delete the %s selected recipes?") % len(
                recs)
        ...
        if de.getBoolean(parent=self.app, label=bigmsg, sublabel=msg, expander=expander):
            ...
            deleterThread.connect(
                'done', lambda *args: self.recTrash.update_from_db())
    ...
    def pause_cb(self, button, *args):
        if button.get_active():
            ...
            self.flusher = GLib.timeout_add(
                1000, lambda *args: self.flush_messages(True))
        else:
