...
class MnemonicManager:
    ...
    def add_ui(self, widgets):
        ...
        for l in more_mnemonics:
            if l not in has_keyval and l not in added:
                has_keyval.append(l)
        for w in has_keyval:
            ...
            if p and isinstance(p.get_parent(), Gtk.Notebook):
                ...
                self.add_widget_mnemonic(
                    w, untouchable=True, fix_untouchables=True)
            else:
    ...
    def find_alternatives(self, w, filter_untouchables=True):
        ...
        while -1 < ind <= last_letter_that_could_be_word_start:
            ...
            ind = text.find(' ', ind+1)
        for l in list(text):
            if l.lower() not in alts and l not in list(' (),_[]:;,.!{}/=+'):
                alts.append(l.lower())
