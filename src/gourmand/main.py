...
import gourmand.batchEditor as batchEditor
import gourmand.convert as convert
import gourmand.plugin as plugin
import gourmand.plugin_gui as plugin_gui
import gourmand.plugin_loader as plugin_loader
import gourmand.prefs as prefs
import gourmand.prefsGui as prefsGui
import gourmand.reccard as reccard
import gourmand.shopgui as shopgui
...
from gourmand.prefs import copy_old_installation_or_initialize, update_preferences_file_format
from gourmand.recindex import RecIndex
from gourmand.threadManager import SuspendableThread, get_thread_manager, get_thread_manager_gui
from gourmand.timer import show_timer

UNDO = 1
SHOW_TRASH = 2


class GourmandApplication:
    """The Gourmand Application.
    ...
    @classmethod
    def instance(cls):
        if GourmandApplication.__single is None:
            GourmandApplication.__single = cls()

        return GourmandApplication.__single
    ...
    def setup_prefs(self):
        ...
        self.prefsGui = prefsGui.PreferencesGui(
            self.prefs, buttons={"clear_remembered_optional_button": lambda *args: self.forget_remembered_optional_ingredients()}
        )
        self.prefsGui.apply_prefs_dic["recipes_per_page"] = lambda p, v: getattr(getattr(self, "rmodel"), "change_items_per_page")(v)
        ...
        # user settings
        toggleFractions(None, self.prefs.get("useFractions", defaults.LANG_PROPERTIES["useFractions"]))

    # Convenience method for showing progress dialogs for import/export/deletion
    def show_progress_dialog(self, thread, progress_dialog_kwargs=None, message=_("Import paused"), stop_message=_("Stop import")):
        ...
        if not hasattr(self, "progress_dialog") or not self.progress_dialog:
            ...
            self.progress_dialog.reassign_buttons(pausecb=progress_dialog_kwargs["pause"], stopcb=progress_dialog_kwargs["stop"])
    ...
    def add_uimanager_to_manage(self, id, uimanager, menu_root_name):
        ...
        self.uimanagers[id] = [uimanager, menu_root_name, {}]  # a dictionary of added menu items
    ...
    def update_action_group(self):
        ...
        for rc in self.rc.values():
            ...
            if not existing_action:
                self.goActionGroup.add_actions([(action_name, None, title, None, None, lambda *args: rc.show())])
            else:
    ...
    def show_about(self, *args):
        ...
        about.set_artists(__version__.artists)
        about.set_authors(__version__.authors)
        about.set_comments(__version__.description)
        about.set_copyright(__version__.copyright)
        # about.set_documenters(None)
        about.set_license(license_text)
        about.set_logo(logo)
        about.set_program_name(__version__.appname)
        about.set_translator_credits(translator)
        about.set_version(__version__.version)
        # about.set_wrap_license(True)
        about.set_website(__version__.url)
        # about.set_website_label('Gourmand website')
    ...
    def quit(self):
        ...
        for r in list(self.rc.values()):
            for c in r.conf:
                c.save_properties()
            if r.edited and de.getBoolean(parent=self.app, label=_("Save your edits to %s") % r.current_rec.title):
        ...
        if len(threads) > 1:
            ...
            quit_anyway = de.getBoolean(label=msg, sublabel=_("Exit program anyway?"), custom_yes=Gtk.STOCK_QUIT, custom_no=_("Don't exit!"), cancel=False)
            if quit_anyway:
                for t in threads:
...
class RecTrash(RecIndex):
    ...
    def setup_main_window(self):
        self.window = Gtk.Dialog(
            ...
            (
                "_Empty trash",
                self.RESPONSE_EMPTY_TRASH,
                "_Delete permanently",
                self.RESPONSE_DELETE_PERMANENTLY,
                "_Undelete",
                self.RESPONSE_UNDELETE,
                Gtk.STOCK_CLOSE,
        ...
        box.pack_start(self.recipe_index_interface, fill=True, expand=True, padding=0)
        self.recipe_index_interface.show()
        self.rg.conf.append(WidgetSaver.WindowSaver(self.window, self.prefs.get("trash_window", {"size": (600, 800)}), show=False))
    ...
    def update_from_db(self):
        self.update_rmodel(self.rg.rd.fetch_all(self.rg.rd.recipe_table, deleted=True))
    ...
    def undelete_selected_recs(self, *args):
        ...
        if msg:
            msg = msg[0:-2]  # cut off the last comma
        self.update_from_db()
        self.rg.redo_search()
...
class UnitModel(Gtk.ListStore):
    def __init__(self, converter):
        ...
        lst = [(a[1][0], a[0]) for a in [x for x in self.conv.units if not (x[1][0] in converter.unit_to_seconds or x[0] in converter.unit_to_seconds)]]
        ##
        lst.sort()
...
def set_path_for_menuitem(mi, base="<main>"):
    if mi.get_children():
        ...
        label = accelLab.get_label().replace("_", "")
        path = base + "/" + label
...
class ImporterExporter:
    ...
    def print_recs(self, *args):
        ...
        printManager.print_recipes(self.rd, recs, parent=self.app, change_units=self.prefs.get("readableUnits", True))
...
class StuffThatShouldBePlugins:
    ...
    def shop_recs(self, *args):
        ...
        for r in rr:
            if r.yields and r.yields != "None":
                ...
                if serv:
                    mult = float(serv) / float(r.yields)
                else:
                    debug("getNumber cancelled", 2)
                    return
            else:
                mult = de.getNumber(default=float(1), label=_("Multiply %s by:") % r.title, parent=self.app.get_toplevel(), digits=2)
                if not mult:
                    debug("getNumber cancelled", 2)
                    return
    ...
    def copy_recipes_callback(self, action: Gtk.Action):
        ...
        ingredients = [self.rd.rd.get_ings(recipe.id) for recipe in recipes]
    ...
    def batch_edit_recs(self, *args):
        ...
        if self.batchEditor.values:
            ...
            msg = ngettext("Set %(attributes)s for %(num)s selected recipe?", "Set %(attributes)s for %(num)s selected recipes?", len(recs)) % {
            ...
            if de.getBoolean(
                label=_("Set values for selected recipes"),
                sublabel=msg,
                cancel=False,
...
class RecGui(RecIndex, GourmandApplication, ImporterExporter, StuffThatShouldBePlugins, plugin_loader.Pluggable):
    ...
    def __init__(self):
        ...
        plugin_loader.Pluggable.__init__(self, [plugin.MainPlugin, plugin.ToolPlugin])
    ...
    def setup_column_display_preferences(self, *args):
        ...
        self.prefsGui.add_pref_table(options, "indexViewVBox", self.configure_columns)
    ...
    def setup_index_columns(self):
        self.rtcolsdic = {}
        self.rtwidgdic = {}
        for a, line, w in REC_ATTRS:
            self.rtcolsdic[a] = line
            self.rtwidgdic[a] = w
            self.rtcols = [r[0] for r in REC_ATTRS]

    def setup_database_hooks(self):
        self.rd.delete_hooks.append(lambda self, *args: (not self.doing_multiple_deletions and self.redo_search()))
    ...
    def setup_main_window(self):
        ...
        saver = WidgetSaver.WindowSaver(self.window, self.prefs.get("app_window", {"window_size": (800, 600)}))
        self.conf.append(saver)

        self.window.set_default_size(*self.prefs["app_window"]["window_size"])
        self.window.set_title(__version__.appname)
        self.main = Gtk.VBox()
        ...
        self.recipe_index_interface = self.ui.get_object("recipeIndexBox")
        self.recipe_index_interface.unparent()
        self.main_notebook.append_page(self.recipe_index_interface, tab_label=Gtk.Label(label=_("Search recipes")))
        ...
        self.rectree.enable_model_drag_source(Gdk.ModifierType.BUTTON1_MASK, [("text/plain", 0, 0)], Gdk.DragAction.COPY | Gdk.DragAction.MOVE)
    ...
    def setup_actions(self):
        ...
        self.onSelectedActionGroup.add_actions(
            [
                ...
                ("DeleteRec", Gtk.STOCK_DELETE, _("Delete recipe"), None, _("Delete selected recipes"), self.rec_tree_delete_rec_cb),
                ("EditRec", Gtk.STOCK_EDIT, _("Edit recipe"), "<Control>E", _("Open selected recipes in recipe editor view"), self.rec_tree_edit_rec),
                (
                    "ExportSelected",
                    None,
                    _("E_xport selected recipes"),
                    "<Control>T",
                    _("Export selected recipes to file"),
                    lambda *args: self.do_export(export_all=False),
                ),
                ("Print", Gtk.STOCK_PRINT, _("_Print"), "<Control>P", None, self.print_recs),
                ("CopyRecipes", Gtk.STOCK_COPY, _("_Copy recipes"), "<Control>C", None, self.copy_recipes_callback),
                ("BatchEdit", None, _("Batch _edit recipes"), "<Control><Shift>E", None, self.batch_edit_recs),
                ("ShopRec", "add-to-shopping-list", _("Add to Shopping List"), "<Control>B", None, self.shop_recs),
            ]
        )
        ...
        self.mainActionGroup.add_actions(
            [
                ("File", None, _("_File")),
                ("New", Gtk.STOCK_NEW, _("_New"), None, None, self.new_rec_card),
                ("Import", None, _("_Import Recipe"), "<Control>M", _("Import recipe from file or page"), self.do_import),
                ("PasteRecipe", Gtk.STOCK_PASTE, _("Paste recipe"), "<Control>V", None, import_from_clipboard),
                (
                    "ExportAll",
                    None,
                    _("Export _all recipes"),
                    "<Control><Shift>T",
                    _("Export all recipes to file"),
                    lambda *args: self.do_export(export_all=True),
                ),
                ("Quit", Gtk.STOCK_QUIT, _("_Quit"), None, None, self.quit),
                ("Actions", None, _("_Actions")),
                ("Edit", None, _("_Edit")),
                ("OpenShoppingList", None, _("_Shopping List"), None, None, lambda action: self.sl.show()),
                ("OpenTrash", None, _("_Trash"), None, None, self.show_deleted_recs),
                ("Preferences", Gtk.STOCK_PREFERENCES, _("_Preferences"), None, None, self.show_preferences),
                (
                    "Plugins",
                    None,
                    _("_Plugins"),
                    None,
                    _("Manage plugins which add extra functionality to Gourmand."),
                    lambda *args: plugin_gui.show_plugin_chooser(),
                ),
                ("Settings", None, _("Setti_ngs")),
                ("HelpMenu", None, _("_Help")),
                ("About", Gtk.STOCK_ABOUT, _("_About"), None, None, self.show_about),
                ("Help", Gtk.STOCK_HELP, _("_Help"), None, None, self.show_help),
            ]
        )
        ...
        self.toolActionGroup.add_actions(
            [
                ("Tools", None, _("_Tools")),
                ("Timer", None, _("_Timer"), None, _("Show timer"), lambda *args: show_timer()),
            ]
        )

        self.goActionGroup.add_actions([("GoRecipeIndex", None, _("Recipe _Index"), None, _("Searchable index of recipes in the database."), self.present)])
    ...
    def rec_tree_delete_rec_cb(self, *args, **kwargs):
        ...
        self.rec_tree_delete_recs(self.get_selected_recs_from_rec_tree())
    ...
    def rec_tree_delete_recs(self, recs):
        ...
        if cancelled:
            for c in cancelled:
                recs.remove(c)
        self.rd.undoable_delete_recs([self.rd.get_rec(r.id) for r in recs], self.history, make_visible=lambda *args: self.redo_search())
        self.setup_delete_messagebox(
            _(
                (
                    f"You just moved {len(recs)} recipe to the trash.\nYou can "
    ...
    def purge_rec_tree(self, recs, paths=None, model=None):
        ...
        if de.getBoolean(parent=self.app, label=bigmsg, sublabel=msg, expander=expander):
            ...
            deleterThread.connect("completed", tmg.notification_thread_done, _("Recipes deleted"))
            tmg.show()
        else:
            return True
