...
class NutritionItemView:
    ...
    def set_unit_visibility(self, visible):
        if visible:
            ...
            if self.unitChoiceLabel:
                self.unitChoiceLabel.show()
            if self.amountLabel:
    ...
    def set_desc_visibility(self, visible):
        if visible:
            ...
            if self.descChoiceLabel:
                self.descChoiceLabel.show()
        else:
            self.descChoiceWidget.hide()
            if self.descChoiceLabel:
                self.descChoiceLabel.hide()
    ...
    def setup_unit_boxes(self, ing=None, nutrow=None):
        ...
        if self.densities:
            units += defaults.VOLUMES[0:]
        if self.extra_units:
            units = list(self.extra_units.keys()) + units
        ...
        for k in list(self.densities.keys()):
            if k:
                ...
                self.setup_choices(
                    list(self.densities.keys()), self.descChoiceWidget)
    ...
    def setup_usda_choices(self, ing):
        ...
        self.setup_choices(
            self.choices[self.usdaChoiceWidget], self.usdaChoiceWidget)
    ...
    def get_multiplier(self, *args):
        ...
        multiplier = self.nut and self.nd.convert_amount(self.amount,
                                                         ...
                                                         d)
        if multiplier:
            # if we don't need unit info, don't show it
            self.set_unit_visibility(False)
            return multiplier
    ...
    def set_amount(self):
        ...
        if not multiplier:
            ...
            # self.unitEntry="?"
            self.currentAmountWidget.set_text(
                "%s %s (? grams)" % (self.amount, self.unit))
        else:
    ...
    def get_new_conversion(self, *args):
        ...
        try:
            ...
            de.show_message(label='Invalid Amount',
                            sublabel='Amount %s is not a number.' % amt)
            self.amountWidget.grab_focus()
        self.amount = amt
        self.unit = unit
        self.usdaChangedCB()
...
class NutritionCardView:
    def __init__(self, recCard):
        ...
        for i in self.ings:
            self.nmodel.add_ingredient(i)
        # initialize our old interface as well...
        NutritionCardViewOld(recCard)

    def get_nd(self):
        if not hasattr(self.rc.rg, 'nutritionData'):
            from . import nutrition
            self.rc.rg.nutritionData = nutrition.NutritionData(
                self.rc.rg.rd, self.rc.rg.conv)

        if hasattr(self.rc.rg, 'nutritionData'):
...
class NutritionCardViewOld:
    ...
    def setup_nmodel(self):
        ...
        for i in self.rc.ings:
            ...
            if aliasrow:
                nut_row = self.rc.rg.rd.nutrition_table.select(
                    {'ndbno': aliasrow.ndbno})
            else:
                nut_row = None
...
class NutritionTreeModel (Gtk.TreeStore):
    ...
    def __init__(self,
                 ...
                 nutrition_fields=['kcal', 'protein', 'carb',
                                   'fasat', 'famono', 'fapoly', 'cholestrl'],
                 ):
    ...
    def build_store(self):
        ...
        for c in self.columns:
            n += 1
            if c in self.numerics:
                self.coltypes += [str]  # everything's a string
            self.ts_col_dic[c] = n
        ...
        for col in self.columns:
            ...
            if col == 'USDA':
                ...
                col = Gtk.TreeViewColumn(
                    col, text_renderer, text=self.ts_col_dic[col])
            col.set_reorderable(True)
            col.set_resizable(True)
