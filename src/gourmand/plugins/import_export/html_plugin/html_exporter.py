...
class html_exporter (exporter_mult):
    def __init__(self, rd, r, out, conv=None,
        ...
        if imagedir and imagedir[-1] != os.path.sep:
            imagedir += os.path.sep  # make sure we end w/ slash
        if not imagedir:
            imagedir = ""  # make sure it's a string
        self.imagedir_absolute = os.path.join(
            os.path.split(out.name)[0], imagedir)
    ...
    def write_inghead(self):
        self.out.write(
            '<div class="ing"><h3>%s</h3><ul class="ing">' % _('Ingredients'))

    def write_text(self, label, text):
        attr = gglobals.NAME_TO_ATTR.get(label, label)
        if attr == 'instructions':
            self.out.write('<div class="%s"><h3 class="%s">%s</h3><div itemprop="recipeInstructions">%s</div></div>' %
                           (attr, label, label, self.htmlify(text)))
        else:
            self.out.write('<div class="%s"><h3 class="%s">%s</h3>%s</div>' %
                           (attr, label, label, self.htmlify(text)))
    ...
    def write_ing(self, amount=1, unit=None,
        ...
        for o in [amount, unit, item]:
            if o:
                self.out.write(xml.sax.saxutils.escape("%s " % o))
        if optional:
...
class website_exporter (ExporterMultirec):
    ...
    def write_header(self):
        self.indexfn = os.path.join(
            self.outdir, 'index%s%s' % (os.path.extsep, self.ext))
        ...
        for r in self.index_rows:
            self.indexf.write('<th class="%s">%s</th>' %
                              (r, gglobals.REC_ATTR_DIC[r]))
            ...
            """<tr><td class="%s">
                     <a href="%s">%s</a>
                               ...
                               xml.sax.saxutils.escape(
                                   self._grab_attr_(rec, self.index_rows[0]))
                               ))
        for r in self.index_rows[1:]:
            self.indexf.write('<td class="%s">%s</td>' %
                              (r, self._grab_attr_(rec, r)))
        self.indexf.write('</tr>')
