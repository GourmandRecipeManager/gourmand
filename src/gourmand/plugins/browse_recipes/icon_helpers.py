...
from gourmand.gtk_extras.ratingWidget import star_generator
from gourmand.image_utils import bytes_to_pixbuf, image_to_pixbuf
...
generic_recipe_image = scale_pb(GdkPixbuf.Pixbuf.new_from_file(
    os.path.join(curdir, 'images', 'generic_recipe.png')))
preptime_image = GdkPixbuf.Pixbuf.new_from_file(
    os.path.join(curdir, 'images', 'preptime.png'))
preptime_empty_image = GdkPixbuf.Pixbuf.new_from_file(
    os.path.join(curdir, 'images', 'preptime_empty_clock.png'))
cooktime_image = GdkPixbuf.Pixbuf.new_from_file(
    os.path.join(curdir, 'images', 'cooktime.png'))
cooktime_empty_image = GdkPixbuf.Pixbuf.new_from_file(
    os.path.join(curdir, 'images', 'cooktime_empty_clock.png'))
cuisine_image = scale_pb(GdkPixbuf.Pixbuf.new_from_file(
    os.path.join(curdir, 'images', 'cuisine.png')))
rating_image = scale_pb(GdkPixbuf.Pixbuf.new_from_file(
    os.path.join(curdir, 'images', 'rating.png')))
source_image = scale_pb(GdkPixbuf.Pixbuf.new_from_file(
    os.path.join(curdir, 'images', 'source.png')))
category_image = scale_pb(GdkPixbuf.Pixbuf.new_from_file(
    os.path.join(curdir, 'images', 'generic_category.png')))
...
def get_recipe_image(rec):
    ...
    big_side = ((pb.get_property('height') > pb.get_property('width')
                and pb.get_property('height')) or pb.get_property('width'))
    if rec.rating:
        ...
        if ratingPB.get_property('width') > pb.get_property('width'):
            SCALE = float(pb.get_property('width')) / \
                ratingPB.get_property('width')
        else:
            SCALE = 1
        ratingPB.composite(
            pb,
    ...
    if rec.preptime:
        ...
        prepPB = prepPB.scale_simple(
            int(big_side*0.4), int(big_side*0.4), GdkPixbuf.InterpType.BILINEAR)
        prepPB.composite(
            pb,
...
class PiePixbufGenerator:
    ...
    def get_image(self, angle, color):
        ...
        d.pieslice((10, 10, ICON_SIZE-10, ICON_SIZE-10), -
                   90, -90 + angle, color)
    ...
    def get_time_image(self, time_in_seconds):
        if time_in_seconds <= 60:
            # seconds (sweep hand)
            return self.get_image((time_in_seconds/60.0) * 360, 'pink')
        elif time_in_seconds <= 60 * 60:
            # minutes (minute hand)
            return self.get_image((time_in_seconds/float(60*60)) * 360, 'red')
