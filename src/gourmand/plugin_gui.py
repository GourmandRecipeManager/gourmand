...
class PluginChooser:
    ...
    def add_labels(self):
        head = Gtk.Label()
        head.set_markup('<b><span size="large">' + _("Plugins") + "</span></b>")
        desc = Gtk.Label()
        desc.set_markup("<i>" + _("Plugins add extra functionality to Gourmand.") + "</i>")
        head.set_alignment(0.0, 0.0)
    ...
    def make_list_store(self, plugin_list) -> Gtk.ListStore:
        # plugin_list = List[Tuple[str, Union["LegacyPlugin", "Plugin"]]]
        ls = Gtk.ListStore(bool, GObject.TYPE_PYOBJECT)  # plugin activated  # plugin and its info
        for module_name, plugin_set in plugin_list:
            ls.append((module_name in self.loader.active_plugin_sets, plugin_set))
        return ls

    @staticmethod
    def plugin_description_formatter(col: Gtk.TreeViewColumn, renderer: Gtk.CellRendererText, mod: Gtk.ListStore, itr: Gtk.TreeIter, data: Any):
        """Format plugin name and description in the plugin window"""
        plugin_set = mod[itr][1]
        renderer.set_property("markup", ("<b>" + escape(plugin_set.name) + "</b>" + '\n<span size="smaller">' + escape(plugin_set.comment) + "</span>"))
    ...
    def do_change_plugin(self, plugin_set, state, ls):
        try:
            ...
            if state:
                de.show_message(message_type=Gtk.MessageType.ERROR, label=_("An error occurred activating plugin."), sublabel=details)
            else:
                de.show_message(message_type=Gtk.MessageType.ERROR, label=_("An error occurred deactivating plugin."), sublabel=details)
