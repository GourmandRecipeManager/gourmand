...
from gourmand.convert import NUMBER_FINDER
from gourmand.gglobals import REC_ATTRS, TEXT_ATTR_DIC
...
class RecHandler (xml_importer.RecHandler):
    ...
    def __init__(self, total=None, conv=None, parent_thread=None):
        xml_importer.RecHandler.__init__(
            self, total, conv=conv, parent_thread=parent_thread)
    ...
    def endElement(self, name):
        if name == 'recipe':
            ...
            self.ing[str(self.ING_ATTRS[name])] = xml.sax.saxutils.unescape(
                self.elbuf.strip())
