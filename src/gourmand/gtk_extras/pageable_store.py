...
class PageableListStore (Gtk.ListStore):
    ...
    def sort(self, column, direction=FORWARD):
        ...
        if direction == self.OFF:
            ...
            return
        self.parent_list.sort(
            key=lambda x: x[column], reverse=(direction == self.REVERSE))
        self.update_tree()
...
class ColumnSortSetterUpper:
    ...
    def set_sort_column_id(self, tree_column, model_column):
        ...
        tree_column.connect(
            'clicked', self.sort_by_column_callback, model_column)
...
if __name__ == '__main__':
    pts = PageableTreeStore([str, str], parent_args=[
                            [str(n), str(30-n)] for n in range(30)])
