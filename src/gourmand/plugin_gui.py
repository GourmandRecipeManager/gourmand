...
class PluginChooser:
    ...
    def add_labels(self):
        ...
        desc = Gtk.Label()
        desc.set_markup(
            '<i>'+_('Plugins add extra functionality to Gourmand.')+'</i>')
    ...
    def make_treeview(self, plugin_list):
        ...
        plugin_col.set_cell_data_func(
            text_renderer, self.plugin_description_formatter)
    ...
    def do_change_plugin(self, plugin_set, state, ls):
        try:
            ...
            if state:
                ...
                de.show_message(message_type=Gtk.MessageType.ERROR,
                                label=_(
                                    'An error occurred deactivating plugin.'),
                                sublabel=details
                                )
