...
class RecCardDisplay (plugin_loader.Pluggable):
    ...
    def setup_uimanager(self):
        ...
        self.ui_manager.insert_action_group(
            self.recipeDisplayFuturePluginActionGroup, 0)
        self.ui_manager.insert_action_group(self.rg.toolActionGroup, 0)
        self.rg.add_uimanager_to_manage(
            self.current_rec.id, self.ui_manager, 'RecipeDisplayMenuBar')

    def setup_actions(self):
        self.recipeDisplayActionGroup = Gtk.ActionGroup(
            name='RecipeDisplayActions')
    ...
    def setup_widgets_from_ui(self):
        for attr in self.__display_items:
            setattr(self, '%sDisplay' %
                    attr, self.ui.get_object('%sDisplay' % attr))
            setattr(self, '%sDisplayLabel' %
                    attr, self.ui.get_object('%sDisplayLabel' % attr))
            try:
        ...
        self.multiplyDisplaySpin.connect(
            'changed', self.multiplication_change_cb)
        self.multiplyDisplayLabel = self.ui.get_object(
            'multiplyByDisplayLabel')
    ...
    def setup_main_window(self):
        ...
        self.window.set_default_size(
            *self.prefs.get('reccard_window_%s' % self.current_rec.id)['window_size'])
        main_vb = Gtk.VBox()
    ...
    def update_from_database(self):
        ...
        for module in self.modules:
            ...
            try:
                ...
                print(
                    'WARNING: Exception raised by %(module)s.update_from_database()' % locals())
                import traceback
                traceback.print_exc()
        ...
        for attr in self.__display_items:
            if attr in special_display_functions:
                ...
                if not widg or not widgLab:
                    raise Exception("There is no widget or label for  %s=%s, %s=%s" % (
    ...
    def update_yields_multiplier(self, val):
        ...
        if yields != self.current_rec.yields:
            ...
            plur_form = defaults.defaults.get_pluralized_form(
                self.current_rec.yield_unit, yields)
        ...
        if float(yields) != self.yields_orig:
            ...
            self.mult = 1
        if self.mult != 1:
            self.yieldsMultiplyByLabel.set_text(
                "x %s" % convert.float_to_frac(self.mult))
        else:
...
class IngredientDisplay:
    ...
    def setup_widgets(self):
        ...
        self.ingredientsDisplayLabel = self.ui.get_object(
            'ingredientsDisplayLabel')
        ...
        for group_index, (group, ings) in enumerate(self.ing_alist):
            ...
            for ing_index, i in enumerate(ings):
                ...
                amt, unit = self.rg.rd.get_amount_and_unit(
                    ...
                    conv=(self.prefs.get('readableUnits', True)
                          and self.rg.conv or None)
                )
                if amt:
                    ing_strs.append(amt)
                if unit:
    ...
    def show_recipe_link_cb(self, widg, link):
        ...
        if rec:
            ...
            de.show_message(parent=self.display_window,
                            label=_(
                                'Unable to find recipe %s in database.') % rname
                            )
...
class RecEditor(WidgetSaver.WidgetPrefs, plugin_loader.Pluggable):
    ...
    def add_plugin(self, klass, position=None):
        ...
        if not position:
            ...
            n = self.notebook.insert_page(
                instance.main, tab_label=tab_label, position=position)
            # We'll need to reset the other plugin's positions if we shoved one in the middle
            for mod in self.modules[position:]:
                self.module_tab_by_name[mod.name] = self.notebook.page_num(
                    mod.main)
        self.module_tab_by_name[instance.name] = n
        ...
        if not position:
            ...
            self.modules = self.modules[:position] + \
                [instance] + self.modules[position:]
        instance.main.show()
        tab_label.show()
        instance.connect('toggle-edited', self.module_edited_cb)
    ...
    def setup_main_interface(self):
        ...
        title = ((self.current_rec and self.current_rec.title)
                 or _('New Recipe')) + ' (%s)' % _('Edit')
        self.window.set_title(title)
        ...
        self.window.set_default_size(
            *prefs.Prefs.instance().get('rec_editor_window')['window_size'])
        main_vb = Gtk.VBox()
        main_vb.pack_start(self.ui_manager.get_widget(
            '/RecipeEditorMenuBar'), expand=False, fill=False, padding=0)
        main_vb.pack_start(self.ui_manager.get_widget(
            '/RecipeEditorToolBar'), expand=False, fill=False, padding=0)
        main_vb.pack_start(self.ui_manager.get_widget(
            '/RecipeEditorEditToolBar'), expand=False, fill=False, padding=0)
    ...
    def set_edited(self, edited):
        ...
        if edited:
            ...
            self.mainRecEditActionGroup.get_action(
    ...
    def notebook_change_cb(self, *args):
        ...
        self.last_merged_ui = self.ui_manager.add_ui_from_string(
            self.modules[page].ui_string)
    ...
    def save_cb(self, action: Gtk.Action):
        ...
        self.widgets_changed_since_save = {}
        self.mainRecEditActionGroup.get_action(
            'ShowRecipeCard').set_sensitive(True)
        self.new = False
...
class IngredientEditorModule (RecEditorModule):
    ...
    def add_ingredient_from_line(self, line, group_iter=None, prev_iter=None):
        ...
        itr = self.ingtree_ui.ingController.add_new_ingredient(
            prev_iter=prev_iter, group_iter=group_iter, **d)
    ...
    def delete_cb(self, *args):
        ...
        rows.reverse()
        self.ingtree_ui.ingController.delete_iters(
            *[mod.get_iter(p) for p in rows])
...
class TextEditor:
    ...
    def setup_action_groups(self):
        self.copyPasteActionGroup = Gtk.ActionGroup(
...
class DescriptionEditorModule (TextEditor, RecEditorModule):
    ...
    def __init__(self, editor: RecEditor):
        ...
        # Attribute names and their widgets
        self.rw: Dict[str, Gtk.Widget] = {}
    ...
    def init_recipe_widgets(self) -> None:
        for attribute, label, widget_type in REC_ATTRS:
            if widget_type == 'Entry':
                ...
                raise ValueError(
                    f"{attribute} with {widget_type} not supported")
        ...
        for e in self.recent:
            if isinstance(self.rw[e], Gtk.SpinButton):
                try:
                    ...
                    debug('%s Value %s is not floatable!' %
                          (e, getattr(self.current_rec, e)))
                    self.rw[e].set_text("")
                Undo.UndoableGenericWidget(
                    self.rw[e], self.history, signal='value-changed')
...
class IngredientController (plugin_loader.Pluggable):
    ...
    def undoable_update_ingredient_row(self, ref, d):
        itr = self.ingredient_editor_module.ingtree_ui.ingController.get_iter_from_persistent_ref(
            ref)
        orig = self.ingredient_editor_module.ingtree_ui.ingController.get_rowdict(
            itr)
        Undo.UndoableObject(
    ...
    def update_ingredient_row(self, iter,
        ...
        if amount is not None:
            self.imodel.set_value(iter, 1, amount)
        if unit is not None:
    ...
    def add_ingredient(self, ing, prev_iter=None, group_iter=None,
        ...
        iter = self._new_iter_(
            prev_iter=prev_iter, group_iter=group_iter, fallback_on_append=fallback_on_append)
    ...
    def add_group(self, name, prev_iter=None, children_iters=[], fallback_on_append=True):
        ...
        for c in children_iters:
            te.move_iter(self.imodel, c, None,
                         parent=groupiter, direction='after')
    ...
    def delete_iters(self, *iters, **kwargs):
        ...
        for itr in iters:
            ...
            undo_info.append(
                (deleted_dic, prev_ref, ing_obj, children, expanded))
    ...
    def _get_undo_info_for_iter_(self, iter):
        ...
        prev_ref = self.get_persistent_ref_from_path(
            prev_path) if prev_path else None
    ...
    def do_undelete_iters(self, rowdicts_and_iters):
        for rowdic, prev_iter, ing_obj, children, expanded in rowdicts_and_iters:
            ...
            if ing_obj and isinstance(ing_obj, str):
                itr = self.add_group(
                    rowdic['amount'], prev_iter, fallback_on_append=False)
            ...
            if expanded:
                self.ingredient_editor_module.ingtree_ui.ingTree.expand_row(
                    self.imodel.get_path(itr), True)
            ...
            if isinstance(ing, str):
                ...
                if not isinstance(ing, (int, RecRef)):
                    ...
                    if ing.deleted:  # If somehow our object is
                        ...
                        d['deleted'] = False
                    if d:
                        self.ingredient_editor_module.rg.rd.modify_ing_and_update_keydic(
                            ing, d)
                else:
                    d['recipe_id'] = self.ingredient_editor_module.current_rec.id
                    self.commited_items_converter[ing] = self.rg.rd.add_ing_and_update_keydic(
                        d)
                    self.imodel.set_value(
                        iter, 0, self.commited_items_converter[ing])
                    # Add ourself to the list of ingredient objects so
                    # we will notice subsequent deletions.
                    self.ingredient_objects.append(
                        self.commited_items_converter[ing])
                return pos+1
        # end commit iter
...
class IngredientTreeUI:
    ...
    def __init__(self, ie, tree):
        ...
        self.rg = self.ingredient_editor_module.rg
        self.ingController = IngredientController(
    ...
    def setup_columns(self):
        ...
        for n, head, tog, model, style, expand in [[1, _('Amt'), False, None, None, False],
            ...
            if tog:
                ...
                renderer.connect(
                    'toggled', self.ingtree_toggled_cb, n, 'Optional')
            ...
            # Non-Toggle setup
                ...
                if head == _('Key'):
                    try:
                        ...
                        debug(
                            'Editing-started connect failed. Upgrade GTK for this functionality.', 0)
                if style:
    ...
    def setup_drag_and_drop(self):
        ...
        self.ingTree.connect('drag-begin',
                             lambda *args: setattr(self, 'ss',
                                                   te.selectionSaver(self.ingTree, 0))
                             )
    ...
    def selection_changed(self, selected=False):
        if selected != self.selected:
            ...
            if hasattr(self.ingredient_editor_module, 'ingredientEditorOnRowActionGroup'):
                self.ingredient_editor_module.ingredientEditorOnRowActionGroup.set_sensitive(
                    self.selected)
    ...
    def ingtree_start_keyedit_cb(self, renderer, cbe, path_string):
        ...
        def vis(m, iter): return m.get_value(iter, 0) and (m.get_value(
            iter, 0) in myKeys or m.get_value(iter, 0).find(itm) > -1)
    ...
    def ingtree_edited_cb(self, renderer, path_string, text, colnum, head):
        ...
        if isinstance(ing, str):
            ...
            if attr == 'amount':
                ...
                amt, msg = self.changeUnit(
                    text, self.ingController.get_rowdict(iter))
                if amt:
                    d['amount'] = amt
                # if msg:
                #    self.re.message(msg)
    ...
    def dragIngsRecCB(self, widget: Gtk.TreeView, context: Any,
        ...
        debug("dragIngsRecCB (self=%s, widget=%s, context=%s, x=%s, y=%s, selection=%s, targetType=%s, time=%s)" % (
            self, widget, context, x, y, selection, targetType, time), 3)
        ...
        if selection.get_target() == self.GOURMET_INTERNAL:
            ...
            for item in self.selected_iters:
                ingredient = self.ingController.get_persistent_ref_from_iter(
                    item)
                selected_iter_refs.append(ingredient)

            def do_move():
                debug('do_move - inside dragIngsRecCB ', 3)
                debug('do_move - get selected_iters from - %s ' %
                      selected_iter_refs, 3)
                diter = self.ingController.get_iter_from_persistent_ref(
                    dref) if dref else None
            ...
            if (position == Gtk.TreeViewDropPosition.BEFORE or
                ...
                pre_path = te.path_next(
                    self.ingController.get_path_from_persistent_ref(dref), -1)
                if pre_path:
                    itr_ref = self.ingController.get_persistent_ref_from_path(
                        pre_path)
                else:
                    itr_ref = None
            else:
                ...
                for l in lines:
                    if group:
                        self.ingredient_editor_module.add_ingredient_from_line(
                            l,
                            group_iter=self.ingController.get_iter_from_persistent_ref(
                                itr_ref)
                        )
                    else:
                        self.ingredient_editor_module.add_ingredient_from_line(
                            l,
                            prev_iter=self.ingController.get_iter_from_persistent_ref(
                                itr_ref)
                        )
    ...
    def ingNewGroupCB(self, *args):
        ...
        undo_info = []
        for i in selected_iters:
            deleted_dic, prev_ref, ing_obj = self.ingController._get_undo_info_for_iter_(
                i)
        ...
        if pitr:
            prev_iter_ref = self.ingController.get_persistent_ref_from_iter(
                pitr)
        else:
            prev_iter_ref = None
        ...
        def do_add_group():
            itr = self.ingController.add_group(
                ...
                prev_iter=(prev_iter_ref and self.ingController.get_iter_from_persistent_ref(
                    prev_iter_ref))
            )
            gi = self.ingController.get_persistent_ref_from_iter(itr)
            self.ingTree.expand_row(
                self.ingController.imodel.get_path(itr), True)
    ...
    def change_group(self, itr, text):
        ...
        obj = Undo.UndoableObject(
            change_my_group, unchange_my_group, self.ingredient_editor_module.history)
        obj.perform()
...
class UndoableTreeStuff:
    ...
    def start_recording_additions(self):
        ...
        self.added = []
        self.pre_ss = te.selectionSaver(
            self.ic.ingredient_editor_module.ingtree_ui.ingTree)
    ...
    def stop_recording_additions(self):
        ...
        self.added = [
            ...
            self.ic.get_persistent_ref_from_iter(
                i.get_model().get_iter(i.get_path()))
            for i in self.added
        ]
    ...
    def record_positions(self, iters):
        ...
        for i in iters:
            ...
            parent_ref = parent and self.ic.get_persistent_ref_from_path(
                parent)
    ...
    def restore_positions(self):
        ...
        for ref, sib_ref, parent_ref in self.positions:
            te.move_iter(self.ic.imodel,
                         ...
                         sibling=sib_ref and self.ic.get_iter_from_persistent_ref(
                             sib_ref),
                         parent=parent_ref and self.ic.get_iter_from_persistent_ref(
                             parent_ref),
                         direction='after'
                         )
...
class IngInfo:
    ...
    def make_key_model(self, myShopCategory):
        ...
        if myShopCategory:
            unique_key_vw = self.rd.get_unique_values(
                'ingkey', table=self.rd.shopcats_table, shopcategory=myShopCategory)
        else:
            #unique_key_vw = self.rd.get_unique_values('ingkey',table=self.rd.keylookup_table)
            unique_key_vw = self.rd.get_unique_values(
                'ingkey', table=self.rd.ingredients_table)
        ...
        try:
            for rec in self.get_selected_recs_from_rec_tree():
                if rec.id == self.re.current_rec.id:
                    ...
                    continue
                amount = YieldSelector(
                    rec, self.re.window).run() if rec.yields else 1
...
class YieldSelector (de.ModalDialog):
    ...
    def __init__(self, rec, parent=None):
        ...
        de.ModalDialog.__init__(
            ...
            label=_('How much of %(title)s does your recipe call for?') % {
                'title': rec.title},
            cancel=False
        )
        self.rec = rec
        ...
        self.recButton.show()
        if rec.yields:
            self.yieldsButton, self.yieldsAdj = self.make_spinny(
                val=self.rec.yields)
