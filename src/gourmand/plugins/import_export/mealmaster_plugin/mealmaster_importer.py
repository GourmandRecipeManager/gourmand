...
class mmf_importer (plaintext_importer.TextImporter):
    ...
    def compile_regexps(self):
        ...
        self.group_matcher = re.compile(
            r"^\s*([M-][M-][M-][M-][M-])-*\s*([^-]+)\s*-*|^\s*---\s*([^-]+)\s*---\s*$", re.IGNORECASE)
        self.ing_cont_matcher = re.compile(r"^\s*[-;]")
        self.ing_opt_matcher = re.compile(
            r"(.+?)\s*\(?\s*optional\)?\s*$", re.IGNORECASE)
        self.ing_or_matcher = re.compile("^[- ]*[Oo][Rr][- ]*$", re.IGNORECASE)
        self.variation_matcher = re.compile(
            r"^\s*(VARIATION|HINT|NOTES?)(:.*)?", re.IGNORECASE)
        ...
        self.amt_field_matcher = re.compile(
            r"^(\s*%s\s*)$" % convert.NUMBER_REGEXP)
    ...
    def handle_line(self, l):
        ...
        if self.is_ingredient(l) and not self.in_variation:
            ...
            add_blank = self.last_line_was == 'blank'
    ...
    def find_ing_fields(self):
        ...
        fields_is_numfield = fields_match(
            all_ings, fields, self.amt_field_matcher)
        ...
        ifield = [fields[0][0], None] if fields else (0, None)
        ...
        if sec_col_fields and len(sec_col_fields) > 2:
            ...
            aindex2, afield2 = self.find_amt_field(
                sec_col_fields, fields_is_numfield)
        ...
        for s, g in self.ingrs:
            for afield, ufield, ifield in ingfields:
                ...
                if not amt.strip() and not u.strip():
                    ...
                    parsed = self.rd.parse_ingredient(
                        i, conv=self.conv, get_key=False)
...
def find_columns(strings, char=" "):
    ...
    columns = [match.start()
               for match in re.finditer(re.escape(char), strings[0])]
    if len(strings) == 1:
        return columns
