...
class ImporterPlugin (StandardPlugin):
    ...
    def test_url(self, url, data, content_type):
        ...
        '''
        tempfilename = self.importManager.get_tempfilename(
...
class DatabasePlugin (StandardPlugin):
    ...
    def activate(self, db):
        ...
        if db._created:
            ...
            try:
                ...
                print(("An InvalidRequestError was caught: {0}".format(
    ...
    def remove(self):
        self.db.remove_hook(plugin_loader.POST,
                            'setup_tables', self.create_tables)
        self.active = False
...
class UIModule:
    ...
    # The label of the tab where this interface goes. (should be i18n'd)
    label = ''
...
class ToolPlugin (UIPlugin):
    ...
    menu_bars = ['RecipeDisplayMenuBar',
                 'RecipeEditorMenuBar', 'RecipeIndexMenuBar']
...
class RecEditorModule (UIModule, GObject.GObject, object):
    ...
    def __init__(self, recEditor):
        ...
        # a list of ActionGroups to be inserted into the uimanager.
        self.action_groups = []
        self.re = recEditor
        self.rg = self.re.rg
    ...
    def setup_main_interface(self):
        self.main = Gtk.Label(
            label='%s Interface not yet implemented' % self.label)
    ...
    def undo_action_callback(self, undo_history, action, typ):
        ...
        prop = (hasattr(widget, 'db_prop')
                and getattr(widget, 'db_prop')) or None
        if prop:
            ...
            if hasattr(widget, 'get_value'):
                ...
                raise TypeError(
                    "I don't know how to get the value from action %s widget %s" % (action, widget))
...
class PrefsPlugin (StandardPlugin):
    ...
    def activate(self, pluggable):
        ...
        if self.label and self.widget:
            self.page_no = self.notebook.append_page(
                self.widget, tab_label=Gtk.Label(label=self.label))
            self.widget.show()
