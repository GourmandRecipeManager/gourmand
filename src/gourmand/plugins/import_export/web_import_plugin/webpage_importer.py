...
import gourmand.importers.importer
...
class WebParser (InteractiveImporter):
    ...
    TAB = '  '
    JOINABLE = ['instructions', 'notes', 'recipe',
                'ignore', 'ingredients', 'include', None]
    INVISIBLE_TYPES = [
    ...
    def commit_rec(self):
        if not self.rec.get('link', ''):
            self.rec['link'] = self.url
        gourmand.importers.importer.Importer.commit_rec(self)
    ...
    def get_images(self):
        ...
        for i in self.soup('img'):
            ...
            if self.imageexcluders:
                exclude = any(exc.search(img_url)
...
class MenuAndAdStrippingWebParser (WebParser):
    ...
    def cut_menus(self):
        menu_regexp = re.compile(
            '.*(menu|nav|search|crumb|sitemap|footer|header).*', re.IGNORECASE)
    ...
    def cut_sponsored_links(self):
        ...
        for spon in spons:
            if hasattr(spon, 'name') and spon.name == 'body':
                continue
...
class WebParserTester (WebParser):
    ...
    def preparse(self):
        ...
        if ings:
            for i in ings:
                self.preparsed_elements.append((i, 'ingredient'))
        author = self.soup('i')
