...
class RecIndex:
        ...
        for c in self.rtcols:
            ...
            # And we also special case our time column
                ...
                if c == 'category':
                    ...
                    list(map(lambda i: model.append(
                        [i]), self.rg.rd.get_unique_values(c)))
    ...
    def search(self, *args):
        ...
        if self.srchentry:
            ...
            debug('Doing new search for %s, last search was %s' %
                  (self.make_search_dic(txt, searchBy), self.last_search), 1)
            GObject.idle_add(lambda *args: (self.do_search(txt,
                             searchBy) or parent.get_window().set_cursor(None)))
        else:
    ...
    def do_search(self, txt, searchBy):
        if txt and searchBy:
            ...
            self.update_rmodel(self.rd.fetch_all(
                self.recipe_table, deleted=False, sort_by=self.sort_by))
    ...
    def limit_search(self, *args):
        ...
        if last_col != _('anywhere'):
            newtext = ' ' + \
                _('%s in %s') % (self.srchentry.get_text(), last_col)
        else:
    ...
    def rtree_time_edited_cb(self, renderer, path_string, text, colnum, attribute):
        ...
        if convert.seconds_to_timestring(getattr(rec, attribute)) != text:
            self.rd.undoable_modify_rec(rec,
                                        {attribute: secs},
                                        self.history,
                                        get_current_rec_method=lambda *args: self.get_selected_recs_from_rec_tree()[
                                            0],
                                        )
    ...
    def get_selected_recs_from_rec_tree(self):
        ...
        def foreach(model, path, iter, recs):
            ...
            try:
                ...
                debug("DEBUG: There was a problem with iter: %s path: %s" %
                      (iter, path), 1)
        recs = []
        sel = self.rectree.get_selection()
        if sel:
