...
# our directory src/lib/plugins/import_export/plugin/*/
test_dir = os.path.split(__file__)[0]
# one back... src/lib/plugins/import_export/plugin/
test_dir = os.path.split(test_dir)[0]
# one back... src/lib/plugins/import_export/
test_dir = os.path.split(test_dir)[0]
...
def assert_equal_ignorecase(val1, val2): return assert_equal(
...
def test_mmf(recs, filename):
    ...
    # test line-wrap
    assert_equal(ings[1].item, 'Finely chopped stuffed green olives')


def test_2_col(recs, filename):
    rd = get_recipe_manager()
    assert len(recs) == 1, 'Expected 1 recipes; got %s (%s)' % (
        len(recs), recs)
