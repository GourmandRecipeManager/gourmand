...
class BatchEditor:
    ...
    def setup_ui(self):
        self.ui = Gtk.Builder()
        self.ui.add_from_string(get_data(
            'gourmand', 'ui/batchEditor.ui').decode())
        self.dialog = self.ui.get_object('batchEditorDialog')
        self.setFieldWhereBlankButton = self.ui.get_object(
            'setFieldWhereBlankButton')
        self.setup_boxes()
        ...
        for a, l, w in gglobals.REC_ATTRS:
            ...
            if checkbutton:
                ...
                if w == 'Combo':
                    ...
                    method = box.get_value if hasattr(
                        box, 'get_value') else box.get_text
                    self.get_data_methods[a] = (checkbutton,
                                                method)
