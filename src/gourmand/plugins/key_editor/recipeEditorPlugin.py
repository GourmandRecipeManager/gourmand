...
class IngredientKeyEditor (RecEditorModule):
    ...
    def setup_main_interface(self):
        ...
        ingredientEditorModule = [
            m for m in self.re.modules if isinstance(m, IngredientEditorModule)][0]
        ingredientEditorModule.connect(
            'saved', lambda *args: self.update_from_database())
        ingredientEditorModule.connect(
            'toggle-edited', self.update_from_ingredient_editor_cb)
        ...
        for plugin in plugin_manager.plugins:
            if plugin.offers_edit_widget():
                ...
                self.extra_widget_table.attach(
                    title_label, 0, 1, ew_index, ew_index+1)
                self.extra_widget_table.attach(
                    widget, 1, 2, ew_index, ew_index+1)
                title_label.show()
                widget.show()
                apply_button.connect(
                    'clicked', lambda *args: plugin.apply_widget_val())
                ew_index += 1
        if ew_index > 1:
            self.extra_widget_table.attach(
                apply_button, 1, 2, ew_index, ew_index+1)
            apply_button.show()
    ...
    def setup_action_groups(self):
        ...
        self.edit_associations_action = self.keyEditorActionGroup.get_action(
            'EditAssociations')
