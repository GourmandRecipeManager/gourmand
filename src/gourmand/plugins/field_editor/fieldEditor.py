...
class FieldEditor:
    ...
    def treeViewSelectionChanged(self, tvSelection):
        ...
        self.valueDialog.set_response_sensitive(
            Gtk.ResponseType.APPLY, (vals and True or False))
    ...
    def dialog_response_cb(self, dialog, response_id):
        ...
        if response_id == Gtk.ResponseType.APPLY:
            ...
            if self.deleteValueButton.get_active():
                label = _('Delete %s where it is %s?') % (
                    self.field, self.val_string)
            ...
            if self.other_field == 'category':
                ...
                for r in self.rd.fetch_all(self.rd.recipe_table, **criteria):
                    if not self.rd.fetch_one(self.rd.categories_table, {'id': r.id}):
                        self.rd.do_add_cat(
                            {'id': r.id, 'category': other_changes['category']})
            else:
                IDs = [r.id for r in self.rd.fetch_all(
                    self.rd.categories_table, **criteria)]
