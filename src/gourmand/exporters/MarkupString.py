...
class simpleHandler (xml.sax.ContentHandler):
    ...
    def startElement(self, name, attrs):
        if name == 'foobar':
            return  # we require an outer wrapper, which we promptly ignore.
...
class MarkupString (str):
    ...
    def __init__(self, string):
        ...
        try:
            xml.sax.parseString("<foobar>%s</foobar>" %
                                str(string), self.handler)
        except:
    ...
    def __getslice__(self, s, e):
        ...
        for el in elements:
            ...
            for k, v in list(attrs.items()):
                stag += " %s=%s" % (k, xml.sax.saxutils.quoteattr(v))
            stag += ">"
        ...
        if e in ends:
            for et in ends[e]:
                outbuf += et
        # the str call is necessary to avoid unicode messiness
        return MarkupString(str(outbuf))
