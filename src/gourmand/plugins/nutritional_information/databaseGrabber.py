...
class DatabaseGrabber:
    ...
    def get_groups(self) -> None:
        ...
        groups = get_data(
            'gourmand', f'data/{self.DESC_FILE_NAME}').decode('iso-8859-1')
        assert groups
    ...
    def parse_line(self, line, field_defs, split_on='^'):
        ...
        for n, fl in enumerate(fields):
            try:
                ...
                print(n, fields[n], 'has no definition in ',
    ...
    def parse_abbrevfile(self, abbrevfile):
        ...
        for n, l in enumerate(ll):
            ...
            if self.show_progress and n % 50 == 0:
                self.show_progress(float(
                    n)/tot, _('Reading nutritional data: imported %s of %s entries.') % (n, tot))
    ...
    def parse_weightfile(self, weightfile):
        ...
        for n, l in enumerate(ll):
            ...
            if self.show_progress and n % 50 == 0:
                self.show_progress(
                    float(n)/tot,
                    _('Reading weight data for nutritional items: imported %s of %s entries') % (
                        n, tot)
                )
