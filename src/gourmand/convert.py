...
class Converter:
    ...
    time_units = [('seconds', [ngettext('second', 'seconds', n) for n in range(5)] + ['s.', 'sec', 'secs', 's']),
                  ...
                  ('minutes', [_('min'), 'min.', 'min', 'mins', 'm'] + \
                   [ngettext('minute', 'minutes', n) for n in range(5)]),
                  # hrs = abbreviation for hours
                  ('hours', [_('hrs.'), 'hrs', 'hr', 'h'] + \
                   [ngettext('hour', 'hours', n) for n in range(5)]),
                  ('days', [ngettext('day', 'days', n) for n in range(5)]),
                  ('weeks', [ngettext('week', 'weeks', n) for n in range(5)]),
                  ('months', [ngettext('month', 'months', n)
                   for n in range(5)]),
    ...
    def adjust_unit(self, amt, unit, item=None, favor_current_unit=True, preferred_unit_groups=[]):
        ...
        try:
            ...
            for u2, rng in units:
                ...
                debug('%s %s, Readability = %s' %
                      (new_amt, u2, readability), 6)
                use_us = False
...
# Is this right -- quick fix here.
NUMBER_END_NO_RANGE_REGEXP = NUMBER_START_REGEXP
NUMBER_MID_REGEXP = NUMBER_REGEXP + ',.' + SLASH
NUMBER_MID_NO_RANGE_REGEXP = NUMBER_MID_REGEXP + " /]"
NUMBER_MID_REGEXP += " /-"
NUMBER_MID_REGEXP += "]"
NUMBER_END_REGEXP = NUMBER_START_REGEXP
NUMBER_REGEXP = "("+NUMBER_START_REGEXP+"*" + \
    NUMBER_MID_REGEXP+"*"+NUMBER_END_REGEXP
...
if NUMBER_WORD_REGEXP:
    ...
    NUM_AND_FRACTION_REGEXP = r"((?P<int>%s)+\s+)?(?P<frac>%s)" % (
        NUMBER_START_REGEXP, FRACTION_REGEXP)
...
# show up sometimes.
# for 'to' used in a range, as in 3-4
RANGE_REGEXP = r'([ -]*%s[ -]*|\s*-\s*)' % _('to')
...
NUMBER_FINDER_REGEXP2 = NUMBER_FINDER_REGEXP.replace(
    'int', 'int2').replace('frac', 'frac2')
...
def float_to_metric(n, approx=0.01):
    ...
    if (n - int(n) < approx) or ((n - int(n) + approx) > 1):
        ...
        return locale.format("%."+str(decimals_to_preserve)+"f", rounded, True)
