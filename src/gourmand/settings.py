...
if getattr(sys, "frozen", False):
    base_dir = op.dirname(sys.executable)
    data_dir = base_dir
    ui_base = op.join(base_dir, "ui")
    doc_base = op.join(base_dir, "doc")
    locale_base = op.join(base_dir, "locale")
    plugin_base = op.join(base_dir)
