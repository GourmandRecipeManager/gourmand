...
class NutritionDataPlugin (DatabasePlugin):
    ...
    def setup_usda_weights_table(self):
        self.db.usda_weights_table = Table('usda_weights', self.db.metadata,
                                           Column('id', Integer(),
                                                  primary_key=True),
                                           *[Column(name, gourmand.backends.db.map_type_to_sqlalchemy(typ), **{})
    ...
    def setup_nutritionconversions_table(self):
        self.db.nutritionconversions_table = Table('nutritionconversions', self.db.metadata,
                                                   Column('id', Integer(),
                                                          primary_key=True),
                                                   Column('ingkey', String(
                                                       length=255), **{}),
                                                   Column('unit', String(
                                                       length=255), **{}),
                                                   # Factor is the amount we multiply
                                                   Column(
                                                       'factor', Float(), **{}),
        ...
        class NutritionConversion (object):
            pass
        self.db._setup_object_for_table(
            self.db.nutritionconversions_table, NutritionConversion)
    ...
    def setup_nutritionaliases_table(self):
        self.db.nutritionaliases_table = Table('nutritionaliases', self.db.metadata,
                                               Column('id', Integer(),
                                                      primary_key=True),
                                               Column('ingkey', Text()),
                                               Column('ndbno', Integer, ForeignKey(
                                                   'nutrition.ndbno')),
        ...
        class NutritionAlias (object):
            pass
        self.db._setup_object_for_table(
            self.db.nutritionaliases_table, NutritionAlias)
    ...
    def create_tables(self, *args):
        ...
        cols = [Column(name, gourmand.backends.db.map_type_to_sqlalchemy(typ), **(name == 'ndbno' and {'primary_key': True} or {}))
    ...
    def update_version(self, gourmand_stored, plugin_stored, gourmand_current, plugin_current):
        if ((gourmand_stored[0] == 0 and gourmand_stored[1] < 14)
            or
                (plugin_stored < 1)):
        ...
        if plugin_stored == '1':
            ...
            self.db.add_column_to_table(self.db.nutrition_table,
                                        ('choline',
                                         gourmand.backends.db.map_type_to_sqlalchemy(
                                             'float'),
                                         {})
                                        )
