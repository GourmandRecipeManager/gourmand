...
class TextImporter (importer.Importer):
    ...
    def compile_regexps(self):
        ...
        self.find_header_breaks_matcher = re.compile(
            r'\s+(?=[A-Z][A-Z][A-Z]+:.*)')
    ...
    def unwrap_lines(self, blob):
        if blob.find("") >= 0:
            ...
            # insert linebreaks where paragraphs were
            outblob = "\n".join(lines)
            return outblob
        outblob = ""
        newline = True
        for l in blob.split('\n'):
            ...
            # else...
            outblob += l.strip()
            # 60 is our hard-coded end-o-paragraph length
            if len(l) < self.end_of_paragraph_length:
                debug('line < %s characters, adding newline.' %
                      self.end_of_paragraph_length, 4)
                outblob += "\n"
                newline = True
            else:
                newline = False
