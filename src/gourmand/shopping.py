...
class Shopper:
        ...
        for a, u, k in inglist:
            ...
            try:
                ...
                if not isinstance(a, tuple):
                    debug(
                        "Warning, can't make sense of amount %s; reading as None" % a, 0)
                    a = None
            if k in dic:
                dic[k].append([a, u])
            else:
            ...
            if not itms:
                ...
                while not flag and len(itms) > ind:
                    ...
                    if isinstance(amt, tuple) or isinstance(a, tuple):
                        ...
                        add_low = self.cnv.add_reasonably(
                            amt[0], unit, a[0], u, ing)
                        add_high = self.cnv.add_reasonably(
                            amt[1], unit, a[1], u, ing)
                        if (not add_low) or (not add_high):
                            ...
                            if add_low:
                                add_low = self.cnv.adjust_unit(
                                    *add_low, **{'favor_current_unit': False})
                            if add_high:
                                add_high = self.cnv.adjust_unit(
                                    *add_high, **{'favor_current_unit': False})
                            if add_low[1] == add_high[1]:  # same unit...
                                add = ((add_low[0], add_high[0]), add_low[1])
                            else:
                                # otherwise, let's use our unit for add_high...
                                u1_to_u2 = self.cnv.converter(
                                    add_low[1], add_high[1])
                        ...
                        if add:
                            ...
                            add = self.cnv.adjust_unit(
                                *add, **{'favor_current_unit': False})
    ...
    def organize(self, dic=None):
        ...
        for c, d in list(cats.items()):
            itms = [[i, self.amt_to_string(amts)]
...
class ShoppingList:
        ...
        for i in ings:
            ...
            if i.optional:
                ...
                if isinstance(include_dic, dict) and (
                    ((i.ingkey not in include_dic)
                     or not include_dic[i.ingkey])
                ):
                    # we ignore our ingredient (don't add it)
                    continue
            amount = self.rd.get_amount(
                i, mult=mult) if self.rd.get_amount(i) else None
            if refid:
    ...
    def doSave(self, filename):
        ...
        with open(filename, 'w') as ofi:
            ...
            def write_itm(a, i): return ofi.write("%s %s" % (a, i) + '\n')
            def write_subh(h): return ofi.write('\n_%s_\n' % h)
    ...
    def _printList(self, printer, *args, **kwargs):
        ...
        def write_itm(a, i): return w.write_paragraph("%s %s" % (a, i))
