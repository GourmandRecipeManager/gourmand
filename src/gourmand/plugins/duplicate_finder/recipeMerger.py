...
class RecipeMergerDialog:
    ...
    def get_widgets(self):
        for w in [
            ...
            # buttons on list-dups page (minus close button)
            'mergeAllButton', 'mergeSelectedButton',
            # buttons on merge-recs page
            'applyMergeButton', 'closeMergeButton', 'cancelMergeButton',
    ...
    def setup_treemodel(self, dups):
        # dup_index, rec_id, rec_title, last_modified, number_of_duplicates
        self.treeModel = Gtk.TreeStore(int, int, str, int, str)
        for dup_index, duplicate_recipes in enumerate(dups):
            ...
            firstIter = self.treeModel.append(
                None,
                (dup_index or 0, first or 0, r.title or '',
                 r.last_modified or 0, str(nduplicates))
            )
            for o in others:
                r = self.rd.get_rec(o)
                self.treeModel.append(firstIter,
                                      (dup_index, o, r.title,
                                       r.last_modified or 0, '')
                                      )
    ...
    def merge_next_recipe(self, ):
        if self.to_merge:
            ...
            last_modified = {'last_modified': [
                r.last_modified for r in self.current_recs]}
            self.current_diff_data = recipeIdentifier.diff_recipes(
                self.rd, self.current_recs)
            last_modified.update(self.current_diff_data)
            self.diff_table = DiffTable(
                last_modified, self.current_recs[0], parent=self.recipeDiffScrolledWindow)
            ...
            if self.recipeDiffScrolledWindow.get_child():
                self.recipeDiffScrolledWindow.remove(
                    self.recipeDiffScrolledWindow.get_child())
    ...
    def do_auto_merge(self, mode):
        ...
        vb.pack_start(l, expand=False, fill=False)
        vb.show_all()
        self.recipeDiffScrolledWindow.add_with_viewport(vb)
        ...
        while self.to_merge:
            ...
            while Gtk.events_pending():
                Gtk.main_iteration()
        self.mergeInfoLabel.set_text(
            'Automatically merged %s recipes' % self.total_to_merge)
        self.closeMergeButton.set_sensitive(True)

    def auto_merge_current_rec(self, mode):
        # TODO make this to an enum and type annotate it
        assert(mode in [NEWER, OLDER])
        self.current_recs.sort(
            key=lambda x: x.last_modified, reverse=(mode == OLDER))
    ...
    def __init__(self, diff_dic, recipe_object=None, parent=None,
        ...
        for attr, name, typ in [('last_modified', 'Last Modified', None)] + gglobals.REC_ATTRS \
            ...
            if attr in diff_dic:
                ...
                label.show()
                self.attach(label, 0, 1, self.row, self.row+1, xoptions=Gtk.AttachOptions.SHRINK |
                            Gtk.AttachOptions.FILL, yoptions=Gtk.AttachOptions.SHRINK | Gtk.AttachOptions.FILL)
                target = None
                for col, b in enumerate(buttons):
                    ...
                    self.attach(b, col+1, col+2, self.row, self.row+1, xoptions=Gtk.AttachOptions.SHRINK |
                                Gtk.AttachOptions.FILL, yoptions=Gtk.AttachOptions.SHRINK | Gtk.AttachOptions.FILL)
                ...
                self.attach(att_label, 0, 1, self.row, self.row+1, xoptions=Gtk.AttachOptions.SHRINK |
                            Gtk.AttachOptions.FILL, yoptions=Gtk.AttachOptions.SHRINK | Gtk.AttachOptions.FILL)
                self.attach(val_label, 1, 5, self.row, self.row+1, xoptions=Gtk.AttachOptions.SHRINK |
                            Gtk.AttachOptions.FILL, yoptions=Gtk.AttachOptions.SHRINK | Gtk.AttachOptions.FILL)
                self.row += 1
        self.mm = mnemonic_manager.MnemonicManager()
    ...
    def add_ingblocks(self, rd, recs):
        ...
        if len(blocks) > 1:
            ...
            for col, block in enumerate(blocks):
                ...
                rb.show()
                self.attach(rb, col+1, col+2, self.row, self.row+1, xoptions=Gtk.AttachOptions.SHRINK |
                            Gtk.AttachOptions.FILL, yoptions=Gtk.AttachOptions.SHRINK | Gtk.AttachOptions.FILL)
        else:
            ...
            l.set_line_wrap_mode(Pango.WrapMode.WORD)
            l.show()
            self.attach(l, 1, 5, self.row, self.row+1, xoptions=Gtk.AttachOptions.SHRINK |
                        Gtk.AttachOptions.FILL, yoptions=Gtk.AttachOptions.SHRINK | Gtk.AttachOptions.FILL)
        lab.set_alignment(0.0, 0.0)
        lab.show()
        self.attach(lab, 0, 1, self.row, self.row+1, xoptions=Gtk.AttachOptions.SHRINK |
                    Gtk.AttachOptions.FILL, yoptions=Gtk.AttachOptions.SHRINK | Gtk.AttachOptions.FILL)
...
if __name__ == '__main__':
    ...
    def test_merger(rd, conflicts):
        ...
        if conflict_dic:
            ...
            print('Differences in ', conflicts,
                  'can be auto-filled with', to_fill)
        else:
