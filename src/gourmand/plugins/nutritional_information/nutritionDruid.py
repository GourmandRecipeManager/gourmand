...
class NutritionUSDAIndex:
    ...
    def __init__(self, rd, prefs, widgets):
        ...
        cb.set_model_from_list(self.foodGroupComboBox,
                               [self.ALL_GROUPS]+self.rd.get_unique_values(
                                   'foodgroup', self.rd.nutrition_table)
                               )
    ...
    def _setup_nuttree_(self):
        ...
        self.nutrition_store = PageableNutritionStore(
            self.rd.fetch_all(self.rd.nutrition_table))
        self.usdaFirstButton.connect(
            'clicked', lambda *args: self.nutrition_store.goto_first_page())
        self.usdaLastButton.connect(
            'clicked', lambda *args: self.nutrition_store.goto_last_page())
        self.usdaForwardButton.connect(
            'clicked', lambda *args: self.nutrition_store.next_page())
        self.usdaBackButton.connect(
            'clicked', lambda *args: self.nutrition_store.prev_page())
        self.nutrition_store.connect(
            'page-changed', self.update_nuttree_showing)
        self.nutrition_store.connect(
            'view-changed', self.update_nuttree_showing)
    ...
    def update_nuttree_showing(self, *args):
        self.usdaShowingLabel.set_text(
            'Showing results %s to %s of %s' % self.nutrition_store.showing())
        # update buttons too
...
class NutritionInfoDruid (GObject.GObject):
    ...
    def __init__(self, nd, prefs, rec=None, in_string=''):
        ...
        self.ui.add_from_string(
            get_data('gourmand', 'ui/nutritionDruid.ui').decode())
    ...
    def _setup_widgets_(self):
        ...
        self.changeKeyButton.connect(
            'clicked', self.changeIngKeyAction.highlight_action)
        ...
        self.changeUnitAction = SpecialAction(highlight_widgets=[self.fromUnitComboBoxEntry,
                                              ...
                                              initially_hidden=True,
                                              hide_on_highlight=[
                                                  self.fromUnitLabel, self.changeUnitButton],
                                              all_controls=self.controls)
        self.changeUnitButton.connect(
            'clicked', self.changeUnitAction.highlight_action)
        self.cancelUnitButton.connect(
            'clicked', self.changeUnitAction.dehighlight_action)
        self.saveUnitButton.connect('clicked', self.save_unit_cb)
        # Nutrition box...
        self.custom_box = self.ui.get_object('customBox')
        self.customNutritionAmountEntry.connect(
            'changed', self.custom_unit_changed)
    ...
    def set_density_info(self, nutalias):
        ...
        others = self.rd.fetch_all(
            self.rd.nutritionconversions_table, ingkey=nutalias.ingkey)
        other_label = '\n'.join('%s: %.1f g' % (
    ...
    def info_edit_equivalent(self, button, eq):
        ...
        conv = self.nd.get_conversion_for_amt(
            1, eq.unit, self.info_nutalias.ingkey)
    ...
    def _setup_custom_box(self):
        ...
        for n, nutstuff in enumerate(NUT_LAYOUT):
            ...
            if show_percent:
                ...
                percent_entry.connect(
                    'changed', self.percent_changed_cb, name, entry)
                percent_entry.entry.set_width_chars(5)
            else:
                percent_entry = None
            entry.connect('changed', self.number_changed_cb,
                          name, percent_entry)
    ...
    def setup_to_units(self):
        ...
        masses = [i[0] for i in defaults.UNIT_GROUPS['metric mass'] +
                  defaults.UNIT_GROUPS['imperial weight']]
        volumes = [i[0] for i in defaults.UNIT_GROUPS['metric volume'] +
        ...
        if not base_convert:
            ...
            if to_unit in self.extra_units:
                ...
                if to_unit.find(' (') > 0:
                    ...
                    raise RuntimeError(
                        "Unable to make sense of conversion from %s %s" % (to_unit, self.ingkey))
                else:
                    density = self.densities[None]
                base_convert = self.nd.conv.converter(
                    'g', to_unit, density=density)
    ...
    def save_unit_cb(self, *args):
        ...
        self.set_from_unit(
            self.fromUnitComboBoxEntry.get_children()[0].get_text())
        self.changeUnitAction.dehighlight_action()
        self.emit('unit-changed', ((old_from_unit, self.ingkey),
                  (from_unit, self.ingkey)))
    ...
    def get_density(self, amount, unit):
        self.densityLabel.set_text(
            _("""In order to calculate nutritional information for "%(amount)s %(unit)s %(ingkey)s", Gourmand needs to know its density. Our nutritional database has several descriptions of this food with different densities. Please select the correct one below.""") % (
        ...
        for d in list(self.densities.keys()):
            group = Gtk.RadioButton(group, str(
                d)+' '+'(%.2f)' % self.densities[d])
    ...
    def edit_nutinfo(self, ingkey=None, desc=None):
        self.amounts[ingkey or desc] = self.get_amounts_and_units_for_ingkey(
            ingkey)
        self.amount_index = 0
        if ingkey:
            self.set_ingkey(ingkey)
    ...
    def check_next_amount(self):
        ...
        self.amount = amount
        self.amount_index += 1
        existing_conversion = self.nd.get_conversion_for_amt(
            amount, unit, self.ingkey, fudge=True)
    ...
    def apply_cb(self, *args):
        ...
        if page == self.NUT_PAGE:
            ...
            if not self.custom_factor:
                de.show_message(
                    _("To apply nutritional information, Gourmand needs a valid amount and unit."))
                return
            self.apply_custom()
            self.check_next_amount()
