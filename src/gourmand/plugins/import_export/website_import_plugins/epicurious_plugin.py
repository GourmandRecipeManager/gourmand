    ...
    def get_importer(self, webpage_importer):
        ...
        class EpicuriousParser(webpage_importer.WebParser):
            ...
            def preparse(self):
                ...
                ingredients_result = self.soup.find_all(
                    'ol', class_='ingredient-groups')[0].find_all('li')
                el_list = []
                for help_result in ingredients_result:
                    if help_result.find('strong'):
                        # This is where ingredient subheading would be.
                        el_list.append(
                            (help_result.find('strong').text, 'ingredients'))
                    playful = help_result.find_all('li')
                    if playful:
                        el_list.extend([(tag.text, 'ingredients')
                                       for tag in playful])
                self.preparsed_elements.extend(el_list)

                direction_list = []
                directions_result = self.soup.find_all(
                    'ol', class_='preparation-groups')
                directions_helpful = directions_result[0].find_all(
                    'li', class_='preparation-group')
                for li in directions_helpful:
                    if li.find('strong'):
                        # This is where direction sub-heading would be
                        direction_list.append(
                            (li.find('strong').text.strip(), 'recipe'))
                    triumph = li.find_all('li', 'preparation-step')
                    for other_li in triumph:
                        direction_list.append(
                            (other_li.text.strip(), 'recipe'))
                ...
                if mod_description:
                    self.preparsed_elements.append(
                        (mod_description.text, 'modifications'))

                mod_chefs_note = self.soup.find(
                    'div', class_='chef-notes-content')
                if mod_chefs_note:
                    self.preparsed_elements.append(
                        (mod_chefs_note.text, 'modifications'))
