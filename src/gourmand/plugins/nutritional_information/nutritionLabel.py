...
class NutritionLabel (Gtk.VBox):
    ...
    def __init__(self, prefs=None,
        ...
        for table, layout in [(self.main_table, MAIN_NUT_LAYOUT),
            ...
            for n, nutstuff in enumerate(layout):
                if nutstuff == SEP:
                    hs = Gtk.HSeparator()
                    table.attach(hs, 0, 2, n+start_at, n+start_at +
                                 1, xoptions=Gtk.AttachOptions.FILL)
                    hs.show()
                    continue
                ...
                if typ == MAJOR:
                    ...
                    permanentl.set_markup(
                        '  <span size="smaller">'+label+'</span>')
                if self.pressable:
                    b = Gtk.Button()
                    b.add(permanentl)
                    b.set_relief(Gtk.ReliefStyle.NONE)
                    b.connect('clicked', self.toggle_label,
                              label, name, properties, unit)
                ...
                if name != 'kcal':
                    ...
                    table.attach(unit_label, 1, 2, n+start_at, n +
                                 start_at+1, xoptions=Gtk.AttachOptions.FILL)
                    unit_label.set_alignment(1, 0.5)
                    unit_label.show()
                hb.show_all()
                table.attach(hb, 0, 1, n+start_at, n+start_at +
                             1, xoptions=Gtk.AttachOptions.FILL)
                if show_percent == SHOW_PERCENT:
                    ...
                    percent_label.show()
                    table.attach(percent_label, 1, 2, n+start_at,
                                 n+start_at+1, xoptions=Gtk.AttachOptions.FILL)
            ...
            for b in list(self.toggles.values()):
                ...
                if b != button:
                    ...
                    if orig.find(self.active_button_markup[0]) == 0:
                        lab.set_markup_with_mnemonic(
                            orig[len(self.active_button_markup[0]):(- len(self.active_button_markup[1]))])
                else:
                    lab.set_markup_with_mnemonic(
                        self.active_button_markup[0]+lab.get_label()+self.active_button_markup[1])
    ...
    def setup_yield_label(self):
        if self.custom_label:
            ...
            self.yieldLabel.set_markup(
                '<b>'+_('Amount per %s' % singular_unit)+'</b>')
        else:
    ...
    def show_druid(self, nd=None, fix_vapor=False):
        ...
        self.ndruid.connect('key-changed', lambda w,
                            tpl: self.emit('ingredients-changed'))
        self.ndruid.connect('unit-changed', lambda w,
                            tpl: self.emit('ingredients-changed'))
        if vapor:
    ...
    class fakenut:
        ...
        def __getattr__(self, n):
            ...
            if n in self.__attdict__:
                ...
                self.__attdict__[n] = self.carb*4+self.sugar * \
                    4+self.protein*4+self.famono*9+self.fasat*9
            else:
