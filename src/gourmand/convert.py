...
class Converter:
    ...
    time_units = [
        ("seconds", [ngettext("second", "seconds", n) for n in range(5)] + ["s.", "sec", "secs", "s"]),
    ...
    def create_unit_dict(self):
        ...
        for u, alts in self.time_units:
            ...
            for a in alts:
                if a not in lst:
                    lst.append(a)
                if a.title() not in lst:
                    lst.append(a.title())
                if a.capitalize() not in lst:
                    lst.append(a.capitalize())
                if a.upper() not in lst:
                    lst.append(a.upper())
                if a.lower() not in lst:
                    lst.append(a.lower())
    ...
    def readability_score(self, amt, unit=None):
        ...
        if unit:
            ...
            try:
                ...
                debug("Key Error for %s in \nunit_group_lookup: %s" % (unit, defaults.unit_group_lookup), 0)
                # raise
                return -10
            else:
    ...
    def adjust_unit(self, amt, unit, item=None, favor_current_unit=True, preferred_unit_groups=[]):
        ...
        try:
            ...
            for u2, rng in units:
                ...
                n1 += 1
            debug("adjust unit called with %s %s, returning %s %s (R:%s)" % (amt, unit, ret_amt, ret_unit, ret_readability), 3)
    ...
    def timestring_to_seconds(self, timestring: str) -> int:
        ...
        for match in NUMBER_FINDER.finditer(timestring):
            if numbers:
                numbers[-1].append(match.start())
            numbers.append([match.start(), match.end()])
        if numbers:
            numbers[-1].append(None)
        secs = 0
...
def seconds_to_timestring(time: int, round_at: Optional[int] = None, fractions: int = FRACTIONS_NORMAL):
    time_strings = []
    ...
    for unit, divisor in units:
        ...
        # minutes.
        add_half = 0
        if time % divisor == 0.5 * divisor and not time_strings:
            add_half = 0.5
            time_covered += 0.5
...
all_number_words.sort(key=cmp_to_key(lambda x, y: ((len(y) > len(x) and 1) or (len(x) > len(y) and -1) or 0)))

NUMBER_WORD_REGEXP = "|".join(all_number_words).replace(" ", r"\s+")
FRACTION_WORD_REGEXP = "|".join([n for n in all_number_words if NUMBER_WORDS[n] < 1.0]).replace(" ", r"\s+")
...
FRACTION_REGEXP = "(" + UNICODE_FRACTION_REGEXP + "|" + DIVIDEND_REGEXP + SLASH_REGEXP + DIVISOR_REGEXP + ")"

AND_REGEXP = r"(\s+%s\s+|\s*[&+]\s*|\s+)" % _("and")

# Match a fraction
if NUMBER_WORD_REGEXP:
    NUM_AND_FRACTION_REGEXP = "((?P<int>%s+|%s)%s)?(?P<frac>(%s|%s))" % (
        NUMBER_START_REGEXP,
        NUMBER_WORD_REGEXP,
        AND_REGEXP,
        FRACTION_REGEXP,
...
for canonical_name, other_names in defaults.UNITS:
    if " " in canonical_name:
        multi_word_units.append(canonical_name)
    for n in other_names:
        if " " in n:
            multi_word_units.append(n)
MULTI_WORD_UNIT_REGEXP = "(" + "|".join([re.escape(str(u)) for u in multi_word_units]) + ")"
...
try:
 ...
 """
    ...
    for s in [
        ...
        try:
            print("DOUBLE CHECK", s, "%%(%s)s" % s % locals())
        except Exception:
            print("Failed with ", s, locals()[s])
    raise

ING_MATCHER = re.compile(ING_MATCHER_REGEXP, re.VERBOSE | re.UNICODE)
...
if __name__ == "__main__" and False:
    ...
    class InteractiveConverter:
        def __init__(self):
            ...
            self.options = {"Convert": self.converter, "Add": self.adder, "Adjust": self.adjuster, "Quit": self.quit}
            while 1:
                self.offer_options()
