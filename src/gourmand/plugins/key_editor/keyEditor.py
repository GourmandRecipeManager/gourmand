...
class KeyEditor:
    ...
    def __init__(self, rd=None, rg=None):
        ...
        self.prev_button.connect(
            'clicked', lambda *args: self.treeModel.prev_page())
        self.next_button.connect(
            'clicked', lambda *args: self.treeModel.next_page())
        self.first_button.connect(
            'clicked', lambda *args: self.treeModel.goto_first_page())
        self.last_button.connect(
            'clicked', lambda *args: self.treeModel.goto_last_page())
        # Setup search stuff
        self.search_string = ""
        ...
        cb.set_model_from_list(
            self.searchByBox, [_('key'), _('item'), _('unit')])
    ...
    def tree_edited(self, renderer, path_string, text, n, head):
        ...
        if field == 'ingkey':
            ...
            if de.getBoolean(label=_('Change all keys "%s" to "%s"?') % (key, text),
                             sublabel=_(
                                 "You won't be able to undo this action. If there are already ingredients with the key \"%s\", you won't be able to distinguish between those items and the items you are changing now." % text)
                             ):
            ...
            if de.getBoolean(label=_('Change all items "%s" to "%s"?') % (curdic['item'], text),
                             sublabel=_(
                                 "You won't be able to undo this action. If there are already ingredients with the item \"%s\", you won't be able to distinguish between those items and the items you are changing now.") % text
                             ):
                self.rd.update_by_criteria(
                    self.rd.ingredients_table,
                    curdic,
            ...
            val = de.getRadio(label='Change unit',
                              options=[
                                  [_('Change _all instances of "%(unit)s" to "%(text)s"') % locals(
                                  ), 1],
                                  [_('Change "%(unit)s" to "%(text)s" only for _ingredients "%(item)s" with key "%(key)s"') % locals(
                                  ), 2],
                              ],
                              default=2,
                              )
            ...
            val = de.getRadio(label='Change amount',
                              options=[
                                  [_('Change _all instances of "%(amount)s" %(unit)s to %(text)s %(unit)s') % locals(
                                  ), 1],
                                  [_('Change "%(amount)s" %(unit)s to "%(text)s" %(unit)s only _where the ingredient key is %(key)s') % locals(
                                  ), 2],
    ...
    def makeTreeModel(self):
        self.treeModel = KeyStore(
            self.rd, per_page=self.rg.prefs.get('recipes_per_page', 12))
            ...
            while child:
                ...
                while grandchild:
                    ...
                    while greatgrandchild:
                        ...
                        keys_to_update[curkey].append(
                            (frac_to_float(amount), unit))
    ...
    def update_showing_label(self):
        ...
        if top >= total and bottom == 1:
            ...
            lab = _(
                'Showing ingredients %(bottom)s to %(top)s of %(total)s' % locals())
        ...
        if field == self.KEY:
            ...
            for item in self.rd.get_unique_values('item', self.rd.ingredients_table, ingkey=ingkey):
                ret.append([None,
                            ...
                            item,
                            self.rd.fetch_len(
                                self.rd.ingredients_table, ingkey=ingkey, item=item),
            ...
            for unit in self.rd.get_unique_values('unit', self.rd.ingredients_table, ingkey=ingkey, item=item):
                ret.append([None,
                            ...
                            unit,
                            self.rd.fetch_len(
                                self.rd.ingredients_table, ingkey=ingkey, item=item, unit=unit),
                            None,
                            ])
            if not ret:
                ret.append([None,
